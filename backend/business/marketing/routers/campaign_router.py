...
from business.marketing.services.skill_router import SkillRouter
from business.learning.service import create_lesson as create_generic_lesson
from business.marketing.schemas import (
    CAMPAIGN_STATUSES,
...
def serialize_experiment(exp: MarketingExperiment) -> Dict[str, Any]:
    return {
        ...
        "assumption_id": str(exp.assumption_id) if getattr(exp, "assumption_id", None) else None,
        "project_id": str(exp.project_id) if getattr(exp, "project_id", None) else None,
        "hypothesis": exp.hypothesis,
        "method": getattr(exp, "method", "ab_test"),
        "metric": exp.metric,
        "success_threshold": getattr(exp, "success_threshold", None),
        "minimum_sample": getattr(exp, "minimum_sample", 0),
        "timebox_days": getattr(exp, "timebox_days", 7),
        "requires_external_action": getattr(exp, "requires_external_action", False),
