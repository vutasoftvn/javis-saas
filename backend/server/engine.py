...
async def single_tool_plan(provider, api_key, model, messages, reasoning, tool_spec):
    ...
    if provider == "openrouter":
        headers.update({"HTTP-Referer": "http://localhost:7777", "X-Title": "COSA OS"})
    payload = {
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                async with client.stream("POST", OPENROUTER_URL, headers=headers, json=payload) as r:
                    ...
                    finish = None
                    usage = None
                    # reasoning_details: khác tool_calls streaming, KHÔNG merge theo index - mỗi
                    # chunk là 1 object reasoning ĐÃ ĐẦY ĐỦ, nối đuôi nhau theo đúng thứ tự nhận
                    # được là ra chuỗi cuối. Model có thể tiếp tục đúng mạch suy luận cũ ở lượt
                    # sau CHỈ KHI mảng này được gửi lại y nguyên, không sắp xếp lại/sửa gì (xem
                    # docs OpenRouter "reasoning tokens" mục preserve reasoning across turns).
                    reasoning_details = []
                    async for line in r.aiter_lines():
                        line = (line or "").strip()
                        if not line.startswith("data:"):
                    ...
                    if reasoning_details:
                        # Caller lưu nguyên mảng này vào message assistant đã lưu lịch sử, rồi gửi
                        # lại y nguyên (key "reasoning_details") ở lượt kế tiếp - payload đã pass-through
                        # thẳng mọi key thừa trong message (xem đầu hàm, payload["messages"] = messages),
                        # nên không cần sửa gì thêm ở chiều gửi đi.
                        yield {"type": "reasoning_details", "content": reasoning_details}
...
async def openrouter_chat_with_mcp(api_key, model, messages, reasoning, mcp_tools, mcp_route):
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json",
               "HTTP-Referer": "http://localhost:7777", "X-Title": "COSA OS"}
    extra = {}
    if reasoning not in (None, "", "off"):
