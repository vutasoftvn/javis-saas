...
def _token_ok(request, path: str) -> bool:
    ...
    try:
        from web_security import verify_jwt
        import sessions
        user_data = verify_jwt(raw)
        if user_data and user_data.user:
            sessions.current_user_id.set(user_data.user.id)
            return True
    except Exception:
        pass

    # Cầu nối auth: token phát hành bởi brain-api mới (cùng cơ chế đã thêm cho WS /ws -
    # xem chú thích ở đó). Flutter gọi REST /sessions.../ bằng Authorization: Bearer token
    # này, nên _auth_guard (middleware HTTP, gọi tới _token_ok) cũng cần nhánh này, không
    # chỉ riêng WS - thiếu nó thì Chat kết nối được nhưng sidebar "Hội thoại" vẫn 401 mãi.
    try:
        import jwt as _pyjwt
        brain_api_secret = os.environ.get("JWT_SECRET", "supersecret-dev-key")
        payload = _pyjwt.decode(raw, brain_api_secret, algorithms=["HS256"])
        user_id = payload.get("sub")
        if user_id:
            import sessions
            sessions.current_user_id.set(user_id)
            return True
    except Exception:
        pass
...
async def do_update():
    ...
    try:
        ...
        port = os.getenv("JAVIS_PORT", "8888")
...
async def websocket_endpoint(ws: WebSocket):
    ...
    if token:
        try:
            ...
            print(f"WS Token auth error: {e}")

    # Cầu nối auth: token phát hành bởi brain-api mới (JWT HS256 ký bằng JWT_SECRET,
    # xem backend/app/core/security.py) - Flutter đăng nhập qua brain-api chứ không
    # còn qua Supabase, nên nhánh verify_jwt ở trên luôn fail cho token này. JWT_SECRET
    # phải khớp giữa 2 tiến trình (backend/.env và brain-api) - xem chú thích trong
    # backend/.env.
    if not is_authenticated and token:
        try:
            import jwt as _pyjwt
            brain_api_secret = os.environ.get("JWT_SECRET", "supersecret-dev-key")
            payload = _pyjwt.decode(token, brain_api_secret, algorithms=["HS256"])
            user_id = payload.get("sub")
            if user_id:
                is_authenticated = True
                import sessions
            ...
            print(f"WS brain-api token auth error: {e}")

    # Fallback to Javis session cookie
...
def _javis_port() -> int:
    try:
        return int(os.getenv("JAVIS_PORT", "8888"))
    except ValueError:
        return 7777
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("JAVIS_PORT", "8888"))
