...
class FakeLongRunningProvider:
    ...
    async def start(self, context, request, idempotency_key):
        from app.workforce.agents.execution.long_running.types import WorkHandle
    ...
    async def poll(self, context, handle):
        from app.workforce.agents.execution.long_running.types import WorkState, WorkStatus
    ...
    async def cancel(self, context, handle):
        from app.workforce.agents.execution.long_running.types import CancelResult, WorkState

        return CancelResult(accepted=True, state=WorkState.CANCELLED)

    async def health(self):
        from app.workforce.agents.execution.long_running.types import WorkProviderHealth

        return WorkProviderHealth(provider_name=self.provider_name, available=True)

    async def capabilities(self):
        from app.workforce.agents.execution.long_running.types import WorkProviderCapabilities
