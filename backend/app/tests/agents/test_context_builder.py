...
def test_build_agent_context_governance_denial_blocks_fetch():
    ...
    with patch(
        "app.workforce.agents.context.builder.GovernanceKernel.evaluate_and_audit_tool_call",
        side_effect=governance_side_effect,
    ), patch(
        "app.workforce.agents.context.builder.get_pipeline_summary"
    ) as mock_pipeline, \
         patch("app.workforce.agents.context.builder.get_financial_summary", return_value={"status": "success", "runway_months": 1}), \
         patch("app.workforce.agents.context.builder.list_okrs", return_value=[]), \
         patch("app.workforce.agents.context.builder.list_projects", return_value=[]):
