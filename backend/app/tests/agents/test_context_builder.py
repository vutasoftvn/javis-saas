...
def test_build_agent_context_assembles_all_sections():
    ...
    with patch("app.agents.context.builder.get_pipeline_summary", return_value={"status": "success", "metrics": {"total_leads": 42}}), \
         patch("app.agents.context.builder.get_financial_summary", return_value={"status": "success", "runway_months": 14.5}), \
         patch("app.agents.context.builder.list_okrs", return_value=[{"id": 1, "name": "Q1 Growth"}]), \
         patch("app.agents.context.builder.list_projects", return_value=[{"id": 2, "name": "Project Apollo"}]):
...
def test_build_agent_context_graceful_degradation_on_tool_failure():
    ...
    with patch("app.agents.context.builder.get_pipeline_summary", return_value={"status": "success", "metrics": {}}), \
         patch("app.agents.context.builder.get_financial_summary", side_effect=crashing_finance_tool), \
         patch("app.agents.context.builder.list_okrs", return_value=[]), \
         patch("app.agents.context.builder.list_projects", return_value=[]):
