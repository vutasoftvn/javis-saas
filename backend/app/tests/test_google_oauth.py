...
def test_tampered_state_is_rejected():
    ...
    state = oauth.sign_state(uuid.uuid4(), uuid.uuid4())
    raw, signature = state.split(".", 1)
    forged = oauth.sign_state(uuid.uuid4(), uuid.uuid4()).split(".", 1)[0]
...
def test_authorize_url_asks_for_offline_access_and_the_right_scopes():
    ...
    url = oauth.build_authorize_url(oauth.sign_state(uuid.uuid4(), uuid.uuid4()))
