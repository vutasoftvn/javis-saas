...
class RunStep(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    run_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("outcome_runs.id"), index=True)
...
class Artifact(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    run_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("outcome_runs.id"), nullable=True, index=True)
