...
def _derive_workspace_key(workspace_id: int) -> bytes:
...
def _get_fernet(workspace_id: int) -> Optional['Fernet']:
    if Fernet is None:
        return None
    key = _derive_workspace_key(workspace_id)
    return Fernet(key)

def encrypt_for_workspace(workspace_id: int, value: str) -> str:
...
def decrypt_for_workspace(workspace_id: int, value: str) -> str:
