...
from workforce.chat.conversation_gate import CanonicalVerb, GateIntent, resolve
from business.sales.models import Contact
...
def test_canonical_ownership_map_exists_and_names_runtime_boundaries():
    ...
    assert "backend/workforce/agents/runtime" in text
...
def test_backend_app_has_no_direct_imports_of_frozen_runtime_scaffolds():
    ...
    production_consumers = [
        ...
        if path.as_posix().startswith("backend/") and "/tests/" not in path.as_posix()
    ]
...
def test_agent_runtime_persistence_models_are_explicit_db_metadata_dependencies():
    root = Path(__file__).resolve().parents[2]
    base = (root / "backend/db/base.py").read_text()
...
def test_workflow_backend_and_frontend_have_one_declared_migration_base():
    ...
    assert "backend/integrations/workflows" in ownership
    assert "frontend/lib/modules/workflows" in ownership

    router = (root / "backend/integrations/workflows/router.py").read_text()
...
def test_invariant_10_no_agent_self_promotion():
    ...
    from workforce.ai.optimization.artifacts import ProgramArtifactStore
