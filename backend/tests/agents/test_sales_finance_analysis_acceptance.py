...
from business.sales.sales_tools import analyze_sales_data
from business.finance.finance_tools import analyze_financial_data
...
async def test_sales_analysis_job_acceptance_flow():
    ...
    with patch("workforce.agents.execution.artifacts.put_object") as mock_put:
...
async def test_chief_of_staff_with_sandbox_csv_analysis(monkeypatch):
    ...
    with patch("workforce.agents.execution.artifacts.put_object"):
        with patch("workforce.agents.orchestration.specialist_registry.get_pipeline_summary", return_value={"total_pipeline": 100000000}):
            with patch("workforce.agents.orchestration.specialist_registry.get_financial_summary", return_value={"runway_months": 8}):
