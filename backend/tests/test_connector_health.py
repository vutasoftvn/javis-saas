...
from integrations.channels.health import check_connector_health
...
async def test_connector_health_rejects_private_or_local_endpoints(monkeypatch):
    monkeypatch.setattr(
        "integrations.channels.health.socket.getaddrinfo",
...
async def test_connector_health_reports_success_for_a_2xx_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("integrations.channels.health.httpx.AsyncClient", lambda **_: Client())
