...
from workforce.memory.health import UNAVAILABLE
from workforce.memory.router import get_memory_health, get_memory_status, get_task_context
...
def test_get_memory_status_reflects_flag_state():
    ...
    with patch("workforce.memory.router.is_enabled", return_value=False):
...
async def test_get_task_context_returns_gateway_result():
    ...
    with patch("workforce.memory.router.get_gateway", return_value=mock_gateway):
