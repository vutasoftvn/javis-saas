...
def test_due_schedule_creates_an_idempotent_copy_of_its_template(monkeypatch):
    ...
    monkeypatch.setattr("founder_os.tasks.scheduler_service.SessionLocal", lambda: db)
    monkeypatch.setattr("founder_os.tasks.scheduler_service.datetime", MagicMock(utcnow=lambda: datetime(2026, 8, 13, 9, 0)))
