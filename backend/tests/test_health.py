...
from bootstrap.create_app import FULL_ROLE, create_app
...
def test_ready_returns_ok_when_db_and_storage_healthy(monkeypatch):
    ...
    monkeypatch.setattr("db.session.engine", _FakeEngine())
    monkeypatch.setattr("integrations.storage.s3_client.get_s3_client", lambda: _FakeS3Client())
    monkeypatch.setattr("core.migration_health.get_migration_health", lambda _engine: (True, "ok"))
    monkeypatch.setattr("core.worker_health.get_worker_health", lambda _engine: (True, "ok"))
...
def test_ready_returns_503_when_schema_is_not_at_head(monkeypatch):
    ...
    monkeypatch.setattr("db.session.engine", _FakeEngine())
    monkeypatch.setattr("integrations.storage.s3_client.get_s3_client", lambda: _FakeS3Client())
    monkeypatch.setattr("core.migration_health.get_migration_health", lambda _engine: (False, "behind"))
    monkeypatch.setattr("core.worker_health.get_worker_health", lambda _engine: (True, "ok"))
...
def test_ready_returns_503_when_worker_has_not_reported_a_heartbeat(monkeypatch):
    ...
    monkeypatch.setattr("integrations.storage.s3_client.get_s3_client", lambda: _FakeS3Client())
    monkeypatch.setattr("core.migration_health.get_migration_health", lambda _engine: (True, "ok"))
    monkeypatch.setattr("core.worker_health.get_worker_health", lambda _engine: (False, "missing"))
...
def test_ready_returns_503_when_database_unreachable(monkeypatch):
    ...
    monkeypatch.setattr("db.session.engine", _FailingEngine())
    monkeypatch.setattr("integrations.storage.s3_client.get_s3_client", lambda: _FakeS3Client())
    monkeypatch.setattr("core.worker_health.get_worker_health", lambda _engine: (True, "ok"))
    monkeypatch.setattr("core.migration_health.get_migration_health", lambda _engine: (True, "ok"))
