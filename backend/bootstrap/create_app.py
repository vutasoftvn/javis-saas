...
def _mount_full_routers(app: FastAPI) -> None:
    ...
    from founder_os.router import router as founder_os_router
    from business.router import router as business_router
    from workforce.router import router as workforce_router
    from integrations.router import router as integrations_router
    from platform_core.router import router as platform_router
    from workforce.agents.capabilities.router import router as capabilities_router
    from workforce.agents.delegation.router import router as delegation_router
...
def _build_lifespan(role: str, *, engine, session_factory):
    if role == FULL_ROLE:
        ...
        async def full_lifespan(app: FastAPI) -> AsyncIterator[None]:
            from integrations.storage.s3_client import ensure_bucket_exists
            from platform_core.sync.entitlement_manager import load_all_current_snapshots_into_cache
...
def _mount_health_probes(app: FastAPI, *, role: str, engine) -> None:
    ...
    def readiness_probe(response: Response):
        ...
        if role == FULL_ROLE:
            from core.migration_health import get_migration_health
            from core.worker_health import get_worker_health
            from integrations.storage.s3_client import get_s3_client
...
def create_app(role: str | None = None) -> FastAPI:
    ...
    from core.runtime_config import validate_runtime_configuration, resolve_cors_origins
