...
class Contact(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True, nullable=False)
    account_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sales.accounts.id"), nullable=True, index=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
...
class SalesLead(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True, nullable=False)
    key_result_id: Mapped[Optional[int]] = mapped_column(ForeignKey("strategy.key_results.id"), nullable=True, index=True)
    account_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sales.accounts.id"), nullable=True, index=True)
    ...
    source: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    source_campaign_id: Mapped[Optional[int]] = mapped_column(ForeignKey("marketing.marketing_campaigns.id"), nullable=True)
    source_experiment_id: Mapped[Optional[int]] = mapped_column(ForeignKey("marketing.marketing_experiments.id"), nullable=True)
...
class SalesOpportunity(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True, nullable=False)
    cycle_id: Mapped[Optional[int]] = mapped_column(ForeignKey("operating.twelve_week_cycles.id"), index=True, nullable=True)
    account_id: Mapped[int] = mapped_column(ForeignKey("sales.accounts.id"), index=True, nullable=False)
    primary_contact_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sales.contacts.id"), nullable=True)
    owner_id: Mapped[Optional[int]] = mapped_column(ForeignKey("core.users.id"), nullable=True)
    source_lead_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sales.sales_leads.id"), nullable=True)
...
class Customer(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True, nullable=False)
    account_id: Mapped[int] = mapped_column(ForeignKey("sales.accounts.id"), nullable=False)
    acquired_from_opportunity_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sales.sales_opportunities.id"), nullable=True)
