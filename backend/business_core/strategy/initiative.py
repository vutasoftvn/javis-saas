...
class Initiative(Base):
    ...
    offering_id: Mapped[Optional[int]] = mapped_column(ForeignKey("core.offerings.id"), nullable=True, index=True)
...
class InitiativeKeyResultLink(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True)
    initiative_id: Mapped[int] = mapped_column(ForeignKey("strategy.initiatives.id"), index=True)
    key_result_id: Mapped[int] = mapped_column(ForeignKey("strategy.key_results.id"), index=True)
