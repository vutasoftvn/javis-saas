...
class SkillImprovementRepository(Protocol):
    ...
    async def get_improvement_request(self, request_id: str) -> SkillImprovementRequest | None: ...

    async def count_requests(
        self, workspace_id: str, skill_id: str, version: str, definition_hash: str
    ) -> int: ...

    async def count_outbox(self, workspace_id: str, request_id: str | None = None) -> int: ...

    async def record_evaluation(self, evaluation: SkillImprovementEvaluationRecord) -> None: ...

    async def get_evaluations(
        self, workspace_id: str, request_id: str
    ) -> list[SkillImprovementEvaluationRecord]: ...

    async def record_mutation(self, mutation: SkillImprovementMutationRecord) -> None: ...
...
class InMemorySkillImprovementRepository:
    ...
    async def get_usage_observations(
        ...
        for obs in self._observations.values():
            if (
                ...
                and obs.run_id == run_id
                and (skill_id is None or obs.skill_id == skill_id)
            ):
                results.append(obs)
        return results
    ...
    async def record_feedback_and_maybe_enqueue(
        ...
        if feedback.run_id:
            matching_obs = await self.get_usage_observations(
                ws_id, feedback.run_id, feedback.skill_id
            )
            if not matching_obs:
                raise ValueError(
                    f"No observation found for run {feedback.run_id} and skill {feedback.skill_id}"
                )
            ...
            if len(distinct_identities) > 1:
                raise ValueError(
                    f"Ambiguous observations for run {feedback.run_id} and skill {feedback.skill_id}"
                )
            obs = matching_obs[0]
        ...
        if idem_key:
            ...
            if lookup_key in self._feedback_by_key:
                ...
                aggs = [
                    ...
                    if a.workspace_id == ws_id
                    and a.skill_id == feedback.skill_id
                    and a.skill_version == version
                    and a.definition_hash == def_hash
                ]
                ...
                reqs = [
                    ...
                    and r.skill_id == feedback.skill_id
                    and r.skill_version == version
                    and r.definition_hash == def_hash
                ]
        ...
        matching_fbs = [
            ...
            and f.skill_id == feedback.skill_id
            and f.version == version
            and f.definition_hash == def_hash
        ]
    ...
    async def claim_improvement_requests(
        ...
        for req in self._requests.values():
            if req.status == "PENDING":
                ...
                req.attempt_count += 1
                claimed.append(
                    ClaimedSkillImprovementRequest(request=req.model_copy(), claim_token=token)
                )
    ...
    async def claim_improvement_outbox(
        ...
        for ob in self._outbox.values():
            if ob.state in ("PENDING", "RETRY") and ob.next_attempt_at <= now:
                ...
                ob.attempt_count += 1
                claimed.append(
                    ClaimedSkillImprovementOutbox(outbox=ob.model_copy(), claim_token=token)
                )
                if len(claimed) >= limit:
                    break
        return claimed
    ...
    async def count_outbox(self, workspace_id: str, request_id: str | None = None) -> int:
        return sum(
            1
            for ob in self._outbox.values()
            if ob.workspace_id == workspace_id
            and (request_id is None or ob.request_id == request_id)
        )
...
class PostgresSkillImprovementRepository:
        ...
        async with self._session_factory() as session:
            if skill_id:
                ...
                rows = await session.execute(
                    stmt, {"ws_id": workspace_id, "run_id": run_id, "skill_id": skill_id}
                )
            else:
                stmt = text(
                    """
    ...
    async def record_feedback_and_maybe_enqueue(
        ...
        async with self._session_factory() as session, session.begin():
            ...
            if feedback.run_id:
                ...
                obs_rows = (
                    await session.execute(
                        stmt,
                        {"ws_id": ws_id, "run_id": feedback.run_id, "skill_id": feedback.skill_id},
                    )
                ).fetchall()
                if not obs_rows:
                    raise ValueError(
                ...
                if (
                    feedback.manifest_hash is not None
                    and obs_row.manifest_hash != feedback.manifest_hash
                ):
                    raise ValueError(
                        f"No observation found matching manifest_hash {feedback.manifest_hash} for run {feedback.run_id}"
                    )
                ...
                raise ValueError(
                    "Skill version and definition hash required when run_id is omitted"
                )
            ...
            if idem_key:
                ...
                existing = (
                    await session.execute(stmt, {"ws_id": ws_id, "idem_key": idem_key})
                ).fetchone()
                if existing:
                    fb_id, v, h = existing
            ...
            fb_rows = (
                await session.execute(
                    fbs_stmt,
                    {
                        ...
                        "skill_id": feedback.skill_id,
                        "def_hash": def_hash,
                        "limit": policy.feedback_window_size,
                    },
                )
            ).fetchall()
    ...
    async def claim_improvement_request(
        ...
        async with self._session_factory() as session, session.begin():
            ...
            await session.execute(
                upd, {"token": token, "worker_id": worker_id, "now": now, "req_id": r.request_id}
            )
            req = SkillImprovementRequest(
                request_id=r.request_id,
                workspace_id=r.workspace_id,
    ...
    async def claim_improvement_outbox(
        ...
        async with self._session_factory() as session, session.begin():
            ...
            for r in rows:
                ...
                await session.execute(
                    upd, {"token": token, "worker_id": worker_id, "ob_id": r.outbox_id}
                )
    ...
    async def count_outbox(self, workspace_id: str, request_id: str | None = None) -> int:
        async with self._session_factory() as session:
            if request_id:
                ...
                return (
                    await session.execute(stmt, {"ws_id": workspace_id, "req_id": request_id})
                ).scalar() or 0
            else:
                stmt = text(
                    """
    ...
    async def get_evaluations(
        ...
        async with self._session_factory() as session:
            ...
            res = await session.execute(
                stmt, {"workspace_id": workspace_id, "request_id": request_id}
            )
            rows = res.fetchall()
            results = []
            for r in rows:
