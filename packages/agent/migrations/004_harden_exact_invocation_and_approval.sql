...
-- agent.run_tool_calls + thêm CAS field cho agent.approvals.
...
-- (packages/agent/capabilities/gateway.py, approval_service.py) tiếp tục
...
-- 1. Đổi FK của agent.approvals TRƯỚC — FK cũ phụ thuộc vào
...
ALTER TABLE agent.approvals DROP CONSTRAINT approvals_tool_call_id_fkey;

-- 2. Đổi PRIMARY KEY của agent.run_tool_calls sang composite (run_id, tool_call_id)
ALTER TABLE agent.run_tool_calls DROP CONSTRAINT run_tool_calls_pkey;
ALTER TABLE agent.run_tool_calls ADD CONSTRAINT run_tool_calls_pkey PRIMARY KEY (run_id, tool_call_id);
ALTER TABLE agent.run_tool_calls ADD CONSTRAINT uq_agent_run_tool_calls_tool_call_id UNIQUE (tool_call_id);

-- 3. Thêm lại FK của agent.approvals theo composite (run_id, tool_call_id)
ALTER TABLE agent.approvals
    ADD CONSTRAINT approvals_run_tool_call_fkey
    FOREIGN KEY (run_id, tool_call_id)
    REFERENCES agent.run_tool_calls(run_id, tool_call_id)
    ON DELETE CASCADE;
...
ALTER TABLE agent.approvals ADD COLUMN IF NOT EXISTS decision_version INTEGER NOT NULL DEFAULT 0;
