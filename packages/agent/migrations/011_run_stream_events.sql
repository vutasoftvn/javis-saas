...
--
-- KHÔNG dùng chung agent.run_events (migration 001) dù tài liệu gốc §7.2
-- đề xuất — phát hiện khi implement: OpenAIAgentsKernel đã tự ghi vào
-- agent.run_events với vocabulary generic riêng của nó (run.started,
-- run.completed{final_output}, run.failed, tool.started, tool.completed,
-- run.waiting, run.resumed — xem packages/agent/kernel/
...
-- vì đây là 2 concern khác nhau: agent.run_events = kernel governance/
-- audit ledger; run_stream_events = app UX fanout log.

-- `sequence` là BIGSERIAL toàn cục (không reset theo từng run_id) — cùng
-- pattern đã proven ở agent.run_events.sequence_no (migration 001):
