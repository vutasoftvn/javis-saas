...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM agent.runs WHERE workspace_id IS NULL LIMIT 1) THEN
        RAISE EXCEPTION 'Cannot migrate: agent.runs has NULL workspace_id';
    END IF;
...
-- ============================================================================
-- Step 2: Remove redundant company_id and tenant_id from agent.runs
-- ============================================================================

DROP INDEX IF EXISTS idx_agent_runs_tenant;
DROP INDEX IF EXISTS idx_agent_runs_company_id;
