...
class GovernanceToCanonicalAdapter:
    """Adapter chuyển đổi dữ liệu lịch sử từ schema prototype `agent_governance.*`
    sang 5 bảng canonical `agent.*` theo Master Guide §12.

    Quy tắc Mapping:
    1. `agent_governance.spec_resolution_manifest_entries`
       -> Đưa vào `manifest_snapshot` của `agent.run_checkpoints`.
    2. `agent_governance.invocation_governance_state`
       -> `governance_state` của `agent.run_tool_calls`.
    3. `agent_governance.invocation_governance_history`
       -> `agent.run_events` (event_type: 'policy.evaluated').
    4. `agent_governance.approval_evidence`
       -> `agent.approvals` (với evidence object).
    """
        ...
        async with self._session_factory() as session:
            ...
            res_manifest = await session.execute(
                text(
                    ...
                    FROM agent_governance.spec_resolution_manifest_entries
                    WHERE run_id = :run_id
                    """
                ),
                {"run_id": run_id},
            ...
            res_states = await session.execute(
                text(
                    ...
                    FROM agent_governance.invocation_governance_state
                    WHERE run_id = :run_id
                    """
                ),
                {"run_id": run_id},
            ...
            res_history = await session.execute(
                text(
                    ...
                    FROM agent_governance.invocation_governance_history
                    WHERE run_id = :run_id
            ...
            for s in state_rows:
                ...
                res_evidence = await session.execute(
                    text(
                        ...
                        FROM agent_governance.approval_evidence
                        WHERE scope = :scope
                        """
                    ),
                    {"scope": tool_call_id},
