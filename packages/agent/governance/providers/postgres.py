...
class PostgresGovernanceStateStore:
    ...
    packages/agent/memory/providers/postgres.py::PostgresMemoryStore
    (constructor nhận db_session_factory, raw SQL qua sqlalchemy.text(), JSON
    serialize thủ công). Schema: agent_governance (xem
    packages/agent/migrations/002_governance_temporal_model.sql)."""
    ...
    async def save_manifest_entry(self, run_id: str, entry: PinnedSpecIdentity) -> None:
        async with self._session_factory() as session:
            await session.execute(
                text(
                    """
                    INSERT INTO agent_governance.spec_resolution_manifest_entries
    ...
    async def save_governance_state(
        ...
        async with self._session_factory() as session:
            await session.execute(
                text(
                    """
                    INSERT INTO agent_governance.invocation_governance_state
            ...
            await session.execute(
                text(
                    """
                    INSERT INTO agent_governance.invocation_governance_history
    ...
    async def load_governance_state(
        ...
        async with self._session_factory() as session:
            result = await session.execute(
                text(
                    """
                    SELECT accumulated FROM agent_governance.invocation_governance_state
    ...
    async def save_evidence(self, evidence: ApprovalEvidence) -> None:
        async with self._session_factory() as session:
            await session.execute(
                text(
                    """
                    INSERT INTO agent_governance.approval_evidence
