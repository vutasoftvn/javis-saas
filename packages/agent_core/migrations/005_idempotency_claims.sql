...
CREATE TABLE IF NOT EXISTS agent_core.idempotency_claims (
    ...
    CONSTRAINT uq_agent_core_idempotency_claims_scope
        UNIQUE (scope_kind, scope_key, capability_id, idempotency_key)
);

CREATE INDEX IF NOT EXISTS idx_agent_core_idempotency_claims_run ON agent_core.idempotency_claims(run_id);
CREATE INDEX IF NOT EXISTS idx_agent_core_idempotency_claims_status ON agent_core.idempotency_claims(status);
