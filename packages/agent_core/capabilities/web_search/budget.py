...
class PostgresWebSearchBudgetStore:
    ...
    async def check_and_consume(
        ...
        *,
        ...
        upsert_stmt = text(
            """
            INSERT INTO agent_core.agent_web_search_budget (
                workspace_id,
                window_start,
                query_count,
                cost_accumulated,
            ...
            DO UPDATE SET
                query_count = agent_core.agent_web_search_budget.query_count + :query_count,
                cost_accumulated = agent_core.agent_web_search_budget.cost_accumulated + :cost,
                updated_at = NOW()
            WHERE
                agent_core.agent_web_search_budget.query_count + :query_count <= agent_core.agent_web_search_budget.daily_query_cap
                AND agent_core.agent_web_search_budget.cost_accumulated + :cost <= agent_core.agent_web_search_budget.daily_cost_cap
