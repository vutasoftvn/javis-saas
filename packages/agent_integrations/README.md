...
- **Cập nhật 2026-09-04:** `AdkCofounderWorkflow` (business pipeline "cofounder mission" cụ thể) không còn tồn tại — nó nằm ở `legacy/agent_runtime/workforce/agents/orchestration/adk/`, và toàn bộ `legacy/` đã bị xoá hẳn 2026-08-25 (xem CLAUDE.md). `docs/agent-platform/ADK_INTEGRATION.md` (tài liệu mô tả pipeline đó) đã xoá theo — không phục hồi. Google ADK hiện tại trong repo chỉ còn `agent_integrations/google_adk/kernel.py` (`GoogleAdkKernel`, adapter GENERIC dùng `google.adk.runners.Runner`/`LlmAgent` thẳng — xem `docs/integrations/google-adk.md`), không có business pipeline cụ thể domain nào đi kèm.
