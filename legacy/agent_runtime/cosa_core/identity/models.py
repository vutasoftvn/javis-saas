...
class Organization(Base):
    ...
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), unique=True, index=True)
...
class Department(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    organization_id: Mapped[int] = mapped_column(ForeignKey("core.organizations.id"), index=True)
    name: Mapped[str] = mapped_column(String(255))
...
class WorkforceMember(Base):
    ...
    human_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("core.users.id"), nullable=True, index=True)
    agent_id: Mapped[Optional[int]] = mapped_column(ForeignKey("runtime_ops.agents.id"), nullable=True, index=True)
...
class DepartmentMembership(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    member_id: Mapped[int] = mapped_column(ForeignKey("core.workforce_members.id"), index=True)
    department_id: Mapped[int] = mapped_column(ForeignKey("core.departments.id"), index=True)
...
class AgentRelation(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    agent_id: Mapped[int] = mapped_column(ForeignKey("runtime_ops.agents.id"), index=True)
    related_member_id: Mapped[int] = mapped_column(ForeignKey("core.workforce_members.id"), index=True)
