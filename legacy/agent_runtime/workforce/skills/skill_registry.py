...
from workforce.models import ToolDefinition, PlatformToolVersion
from workforce.registry.defaults import DEFAULT_TOOL_MANIFESTS
from core.snowflake import generate_snowflake_id


class SkillRegistryService:
    """Service quản lý kho kỹ năng (Skill & Tool Registry) trong COSA."""

    def __init__(self, db: AsyncSession):
        self.db = db

    async def get_tool(self, key: str, workspace_id: Optional[int] = None) -> Optional[ToolDefinition]:
    ...
    async def register_tool(
        ...
        existing = await self.get_tool(key, workspace_id)
        if existing:
            existing.name = name
        ...
        tool = ToolDefinition(
            ...
            default_config_jsonb=config or {},
        )
        self.db.add(tool)
        await self.db.flush()
        return tool
    ...
    async def seed_factory_defaults(self, workspace_id: Optional[int] = None) -> List[ToolDefinition]:
        seeded = []
        for m in DEFAULT_TOOL_MANIFESTS:
            t = await self.register_tool(
                key=m["key"],
                name=m["name"],
