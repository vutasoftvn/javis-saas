...
class BscScorecard(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True)
    strategy_profile_id: Mapped[int] = mapped_column(ForeignKey("strategy.strategy_canvases.id"), index=True)
...
class StrategicObjective(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True)
    scorecard_id: Mapped[int] = mapped_column(ForeignKey("strategy.bsc_scorecards.id"), index=True)
...
class StrategicObjectiveLink(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True)
    from_objective_id: Mapped[int] = mapped_column(ForeignKey("strategy.strategic_objectives.id"), index=True)
    to_objective_id: Mapped[int] = mapped_column(ForeignKey("strategy.strategic_objectives.id"), index=True)
