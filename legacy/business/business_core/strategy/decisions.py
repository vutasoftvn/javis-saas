...
class StrategicDecision(Base):
    ...
    context_pack_id: Mapped[Optional[int]] = mapped_column(ForeignKey("strategy.context_packs.id"), nullable=True, index=True)
...
class GateDecision(Base):
    ...
    __table_args__ = (Index("ix_gate_decision_mvp_stage_id", "mvp_stage_id"), {"schema": "strategy"})
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=False, default=generate_snowflake_id)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("core.workspaces.id"), index=True)
    project_id: Mapped[int] = mapped_column(ForeignKey("strategy.projects.id", ondelete="CASCADE"), index=True)
    milestone_id: Mapped[Optional[int]] = mapped_column(ForeignKey("operating.milestones.id", ondelete="SET NULL"), nullable=True, index=True)
    stage_id: Mapped[Optional[int]] = mapped_column(ForeignKey("operating.cycle_stages.id", ondelete="SET NULL"), nullable=True, index=True)
    # MVP-stage Week 13 gate decisions set this instead of stage_id (CycleStage).
    mvp_stage_id: Mapped[Optional[int]] = mapped_column(ForeignKey("strategy.mvp_stages.id", ondelete="SET NULL"), nullable=True)
    ...
    decided_by: Mapped[int] = mapped_column(ForeignKey("core.users.id"))
