...
from workforce.chat.worker_prompt import WorkerPromptResult
from founder_os.strategy.models import MvpStage
...
_MODULE = "founder_os.strategy.project_orchestration_service"
...
def test_generate_roadmap_never_calls_a_provider_from_brain_api():
    ...
    with patch(f"{_MODULE}.is_provider_configured", return_value=True), \
         patch(f"{_MODULE}.run_worker_prompt_sync", side_effect=_worker_reply(ai_response)), \
         patch(f"{_MODULE}.fetch_foundation_context", return_value=None), \
         patch("workforce.chat.providers.build_provider", side_effect=_explode):
        draft = service.generate_roadmap(project_id)
...
def test_generate_roadmap_prompt_includes_approved_foundation():
    from db.models import Brain, User, Workspace, WorkspaceMember
    from db.session import SessionLocal
    from founder_os.strategy.models import CoreValue, Project, StrategyCanvas, StrategyFoundation, StrategyRevision
...
def test_save_roadmap_draft_persists_stages_as_draft_status():
    from founder_os.strategy.schemas.project_orchestration_schemas import RoadmapDraft
...
def test_save_then_confirm_roadmap_writes_vault_artifact_and_audit_event():
    from db.models import Brain, User, Workspace, WorkspaceMember
    from db.session import SessionLocal
    from founder_os.strategy.models import Project, StrategyAuditEvent
    from founder_os.strategy.schemas.project_orchestration_schemas import RoadmapDraft
...
def _approved_plan():
    from founder_os.strategy.schemas.project_orchestration_schemas import StagePlanDraft
...
def _setup_confirmed_stage():
    from db.models import Brain, User, Workspace, WorkspaceMember
    from db.session import SessionLocal
    from founder_os.strategy.models import MvpStage, Project
    from founder_os.strategy.template_service import TemplateService
...
def test_material_revision_preview_supersedes_unstarted_assignments_and_preserves_evidence():
    from founder_os.strategy.routing_service import RoutingService
    ...
    try:
        ...
        with patch("founder_os.strategy.routing_service.is_provider_configured", return_value=False):
...
def test_apply_revision_rejects_a_stale_preview():
    from founder_os.strategy.schemas.project_orchestration_schemas import StageRevisionChange
