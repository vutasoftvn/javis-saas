...
from integrations.channels.models import Chatbot, EmailApproval, Outbox
from integrations.workflows.n8n_gateway_service import (
    dispatch_n8n_workflow,
    generate_hmac_signature,
    handle_n8n_callback,
)
from integrations.channels.outbox.outbox_processor import process_single_outbox_item
...
async def test_outbox_zalo_fallback_to_telegram():
    ...
    with patch("integrations.channels.outbox.outbox_processor.send_zalo_oa_message", side_effect=Exception("Zalo token expired")):
        with patch("integrations.channels.outbox.outbox_processor.send_telegram_message", new_callable=AsyncMock) as mock_tg:
...
async def test_outbox_email_resend_actually_sends_before_marking_sent():
    ...
    with patch(
        "integrations.channels.email.providers.resend_provider.build_resend_client",
...
async def test_outbox_email_gmail_draft_send_calls_real_client():
    ...
    with patch(
        "integrations.channels.google.google_connection_service.build_gmail_client",
