...
def test_control_plane_deployments_table_does_not_collide_with_local_business_db():
    ...
    __tablename__ = 'deployments', schema `core` sau khi tach business core
    schemas) trung ten voi bang control-plane 'deployments' (VPS deployment
    registry). Ca 2 phai la 2 Table object khac nhau, khac schema, khac cot."""
    code = """
from platform_core.control_plane.db import ControlPlaneBase
import platform_core.control_plane.models  # noqa: F401
...
local_deployments = LocalBase.metadata.tables["core.deployments"]

assert cp_deployments is not local_deployments
assert cp_deployments.schema == "control_plane"
assert local_deployments.schema == "core"
