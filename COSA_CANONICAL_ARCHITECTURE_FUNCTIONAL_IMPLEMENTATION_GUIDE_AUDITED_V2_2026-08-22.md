...
> **Revision:** 2026-08-22 — audited v2: giữ Text Chat như first-class primary channel và bổ sung 4 kết quả audit độc lập: `agentos/improvement/`, RBAC decision kernel, trạng thái `agentos/api/`, và mức hoàn thiện thực tế của Memory/Knowledge.
...
### 0.4. Audit corrections bắt buộc áp dụng từ revision này

Revision này thay thế các claim trước đó ở bốn điểm đã được kiểm tra trực tiếp trên `main@57c6c2c3d8581dcb4c879b18e26e24137eb5926d`:

1. **`agentos/improvement/` không phải code chết.** Đây là implementation có chủ đích của Self-Improvement loop, có test riêng và test end-to-end. Trạng thái đúng là `IMPLEMENTED / TESTED / NOT YET WIRED TO PRODUCTION EVAL PIPELINE`; phải **KEEP + document**, không xóa.
2. **RBAC Phase 1 chưa có intersection thật trong AgentOS.** `PolicyEngine.evaluate_for_agent()` hiện mới xét Agent PermissionLevel × ToolRisk × ToolPermission; RBAC phải được bổ sung như decision input độc lập.
3. **`agentos/api/` hiện không tồn tại.** AgentOS hiện không có HTTP/SSE entrypoint production. Tuy nhiên `legacy/agent_runtime/workforce/api/` và `legacy/agent_runtime/workforce/chat/` là migration reference quan trọng; vì vậy đây là **greenfield current API surface, không phải greenfield behavioral design**.
4. **Memory và Knowledge có mức trưởng thành khác nhau.** Knowledge đã có chunk → embed → store → semantic retrieve ở mức unit-testable core; pgvector chỉ mới ở adapter-SQL/fake-session level, chưa có migration/integration thật. Memory chưa có `providers/`, `PgVectorMemoryStore` hiện không lưu vector và không semantic-search, default runtime cũng chưa wire memory/knowledge.

Mọi implementation plan phải dùng các trạng thái trên thay cho wording tổng quát như “đã có pgvector” hoặc “đã có API” nếu chưa nói rõ mức verification.
...
| Agent API | Python AgentOS API boundary | CURRENT ABSENT → TARGET | `agentos/api/` is greenfield current surface; selective legacy behavior migration only |
...
| Vector extension | pgvector | INFRA AVAILABLE / PERSISTENCE NOT VERIFIED | Knowledge SQL adapter exists; real Knowledge migration/integration pending; Memory currently non-vector |
...
| Knowledge | AgentOS core + pgvector adapter | CURRENT CORE / NOT PRODUCTION-WIRED | chunk/embed/retrieve exists; parser, migration, real pgvector integration and AgentContext wiring pending |
| Agent memory | `MemoryStore` + retriever/consolidation | CURRENT PROTOTYPE → TARGET PROVIDERS | `providers/` absent; current pgvector-named store is not semantic vector memory |
...
`agentos/core/policy.py` đã có `PermissionLevel`, `PolicyDecision`, `PermissionClass` và `evaluate_for_agent()`, nhưng hàm này hiện chỉ kết hợp:

```text
AgentPermissionLevel × ToolRisk × ToolPermission
```

Nó **chưa nhận RBAC role**, và rule hiện tại coi `critical` là `REQUIRE_APPROVAL` trước các nhánh khác. Vì vậy chưa thể thỏa acceptance `Founder + CRITICAL + L3 = ALLOW`.

### TARGET — minimum decision kernel của Phase 1

Phase 1 chỉ implement intersection tối thiểu:
...
`TenantPolicy`, `ExecutionMode` và `DataScope` **không đưa vào Phase 1**; chúng được bổ sung ở governance phase sau (Phase 10 trong implementation rollout) để tránh làm decision kernel đầu tiên quá rộng.
...
RBAC role phải được resolve từ canonical Control Plane/TenantContext; client hoặc model không được tự khai role. AgentOS không sở hữu role catalog.

Acceptance tối thiểu:

```text
Founder + CRITICAL + L3              → ALLOW
Member  + CRITICAL + L2              → REQUIRE_APPROVAL
Auditor + any non-read operation      → DENY
```

Nếu database tiếp tục dùng role id `user` thay vì `member`, policy boundary phải normalize vocabulary rõ ràng. Nếu dùng `auditor` trong sản phẩm thì thêm role bằng migration mới trong Control Plane, không sửa migration lịch sử.

Hard-deny RBAC (ví dụ Auditor ghi dữ liệu) phải thắng mọi mức autonomy của agent. `PermissionClass` hiện tại được giữ như metadata/migration vocabulary trong giai đoạn chuyển đổi; nó không phải decision mechanism cuối cùng duy nhất.
...
## 9.3. Self-Improvement subsystem — ownership đã audit

### CURRENT

`agentos/improvement/` tồn tại và gồm các primitive cho Self-Improvement loop như capability-gap detection, proposal/approval gate, improvement hierarchy và skill distillation. Module này có test suite riêng và có integration test chứng minh flow:

```text
failure/eval evidence
  → capability gap
  → candidate skill
  → human approval
  → SupplyChainPipeline
...
Commit provenance cho thấy đây là Phase 10 Self-Improvement được implement có chủ đích rồi được giữ lại khi AgentOS được restructure từ `backend/agentos` lên root `agentos/`.

**Quyết định:** `KEEP`. Không xóa như code chết.

Operational status chuẩn:

```text
IMPLEMENTED
TESTED
NOT YET WIRED TO PRODUCTION EVAL PIPELINE
```

Gap còn lại là wiring `CapabilityOutcome` từ eval/run history thật; subsystem hiện nhận evidence do caller cung cấp.

### Documentation requirement

Bổ sung `agentos/improvement/README.md` và một row trong Ownership Map:

```text
Capability: Agent Self-Improvement / Capability Improvement
Canonical owner: agentos/improvement/
Operational status: IMPLEMENTED / TESTED / NOT YET PRODUCTION-WIRED
```

Self-improvement không được tự sửa core code bỏ qua `ImprovementLevel` hierarchy hoặc approval/supply-chain gate.

## 9.4. Runtime lựa chọn
...
# 14. Memory Provider — audited current state và target mở rộng

## 14.1. CURRENT — không được gọi là production semantic memory

`agentos/memory/providers/` **chưa tồn tại**. Current package gồm `models.py`, `store.py`, `pgvector_store.py`, `retrieval.py`, `retriever.py`, `consolidation.py`. Vì vậy mọi cấu trúc `providers/*` bên dưới là TARGET, không phải CURRENT.

Đã có:

- `MemoryStore` protocol với `put/search/delete`;
- `InMemoryMemoryStore`, có unit tests và workspace/kind filtering;
- `MemoryRetriever`, có hook inject vào `ContextBuilder`;
- consolidation primitives;
- `PgVectorMemoryStore` như một persistence adapter thử nghiệm.

Nhưng cần ghi đúng bản chất: **`PgVectorMemoryStore` hiện không có cột embedding, không dùng pgvector operator và không semantic-search**. Search hiện chỉ filter `workspace_id/agent_key/kind` rồi `ORDER BY created_at DESC`. Tên class đang mạnh hơn capability thực tế.

Memory relevance hiện cũng chỉ dùng lexical term-overlap (`[a-z0-9]+`) kết hợp importance; chưa phải semantic retrieval và chưa có recency factor thực tế dù một số docstring cũ nói “recency”. Với tiếng Việt, đây chỉ là placeholder.

Một P0 reliability gap: nếu `db_session_factory` không được cấu hình, `PgVectorMemoryStore.put/delete` có thể return im lặng và `search` trả `[]`. Production không được chấp nhận silent persistence loss như vậy.

`build_default_runtime()` hiện cũng **không wire `MemoryStore/MemoryRetriever`**; vì vậy memory mới là integration hook, chưa phải default production path.

## 14.2. TARGET service boundary

Nên tách high-level service semantics khỏi low-level storage protocol:
...
└── namespace/isolation
      ...
      ├── Postgres/Vector Memory Backend
      └── TencentAgentMemoryBackend (optional)
```

Target organization có thể là:

```text
agentos/memory/providers/
├── in_memory.py
├── postgres.py
├── pgvector.py              # chỉ khi có vector semantics thật
└── tencent_agent_memory.py  # optional adapter
```

Không ép external Agent Memory service giả làm SQL store nếu semantics khác. `MemoryStore` có thể tiếp tục làm low-level port; `MemoryService` là canonical agent-facing contract.

## 14.3. Khi thêm Memory Provider
...
- retention/forget;
- relevance ranking;
- semantic retrieval nếu provider claim vector capability;
- offline behavior;
- failure behavior không silent no-op;
- migration/backup story;
- deterministic configuration failure khi persistence dependency thiếu.

## 14.4. DB ownership cần chốt trước production

Recommended direction cho local-first COSA:

```text
One PostgreSQL infrastructure
├── business/control schemas  ← services/* owns
├── agent_memory schema       ← agentos/memory owns
└── knowledge schema          ← agentos/knowledge owns
```

Tách **schema ownership**, không cần tạo một Postgres server thứ hai chỉ để tách AgentOS ở giai đoạn hiện tại. Nếu chọn mô hình khác phải có ADR.

## 14.5. Không silent no-op

Production backend không được kiểu:

```text
no session factory → put returns silently
```

Phải fail fast ở startup/configuration hoặc expose explicit unavailable mode chỉ dành cho test/offline profile.

---

# 15. Knowledge Source — audited RAG productionization

## 15.1. CURRENT — functional core đã có, production persistence chưa có

Knowledge mạnh hơn Memory một bậc. Current AgentOS đã implement và unit-test được:

```text
raw_text
  ↓
character chunking + overlap
  ↓
EmbeddingProvider
  ↓
...
- `KnowledgeSource`, `KnowledgeChunk`, `KnowledgeSearchResult`;
- character-based `chunk_text()` (`800` chars, `100` overlap mặc định);
- `EmbeddingProvider`;
- `OpenAICompatibleEmbeddingProvider` gọi `/embeddings` thật qua `httpx`;
- deterministic `StubEmbeddingProvider`;
- `KnowledgeIngestPipeline`;
- `KnowledgeRetriever`;
- `InMemoryKnowledgeStore` với cosine similarity thật;
- `PgVectorKnowledgeStore` có SQL `<=>` cosine-distance.

Nhưng phải phân biệt rõ:

```text
PgVector adapter SQL exists
        ≠
real PostgreSQL/pgvector persistence verified
```

`PgVectorKnowledgeStore` hiện chỉ được test với fake session để kiểm SQL/params/row mapping. Chưa có real DB migration/integration test cho `knowledge_sources`, `knowledge_chunks`, `CREATE EXTENSION vector`, vector dimension, index hoặc upgrade/downgrade.

## 15.2. Parse layer thực tế chưa có

`KnowledgeIngestPipeline.ingest()` nhận `raw_text`; caller phải tự parse file. Vì vậy flow hiện tại là:
...
Chưa có canonical PDF/DOCX/HTML parser pipeline trong `agentos/knowledge/`.

## 15.3. Knowledge chưa được wire vào Agent Context/default runtime

`ContextBuilder` hiện có memory/skills hooks nhưng chưa có `KnowledgeRetriever`; `build_default_runtime()` cũng chưa inject Knowledge. Vì vậy current AgentOS chưa phải production RAG path dù Knowledge subsystem độc lập đã có logic retrieval thật.

TARGET composition phải đưa Knowledge qua cùng context pipeline của Text Chat/Voice/Agent Runtime, không để channel tự query vector store riêng.

## 15.4. Target production pipeline
...
## 15.5. Flow thêm source

1. define source type;
2. define parser;
3. preserve source metadata/provenance;
4. choose chunk strategy;
5. pin embedding model/version/dimensions;
6. tenant/workspace scope;
7. upsert/idempotency + content hash;
8. delete/reindex flow;
9. retrieval eval;
10. citations/provenance;
11. integration test against real pgvector before marking production-ready.

Không cho LLM ingest file rồi bỏ mất source/metadata.

## 15.6. Hardening bắt buộc

- validate `len(embeddings) == len(chunks)` trước `zip`, tránh silent truncation nếu provider trả thiếu vector;
- lưu `embedding_model`, `embedding_dimensions`, `embedding_version`/index generation;
- lưu `content_hash`, parser/version metadata để re-index an toàn;
- define deletion cascade/source lifecycle;
- enforce workspace isolation ở DB query và integration tests;
- production store không được silent no-op khi session factory thiếu.
...
### 17.1.1. CURRENT Agent API status — greenfield surface, legacy reference exists

Audit xác nhận `agentos/api/` **chưa tồn tại** và current `agentos/requirements.txt` cũng không mang một HTTP/SSE server framework cho AgentOS entrypoint. `services/` cũng không có một hidden `agent-api/chat` service tương đương.

Vì vậy:

```text
Current AgentOS HTTP/SSE API implementation = 0%
```

Nhưng không được hiểu là toàn bộ behavior phải thiết kế từ số 0. `legacy/agent_runtime/workforce/api/` và đặc biệt `legacy/agent_runtime/workforce/chat/` có migration references đáng giữ về:

- `ChatTurn`, `ToolCall`, `AIEvent` style contracts;
- async streaming provider contract;
- `delta/completed/failed/tool_call` semantics;
- worker ↔ API streaming lifecycle;
- monotonic offset / reconnect / resync lessons;
- Postgres LISTEN/NOTIFY approach với DB là source of truth;
- conversation gating lessons.

Không port trực tiếp legacy FastAPI/SQLAlchemy/auth imports, direct business DB access, duplicate model registry hoặc duplicate tool-execution stack.

Canonical classification:

> **Greenfield API surface over the current AgentOS kernel, informed by a legacy chat/API implementation that must be selectively migrated rather than resurrected.**

### 17.1.2. Agent Chat API target
...
### 17.1.3. Streaming event contract
...
## Phase 1 — AgentOS Production Composition + Minimum RBAC Kernel

- canonical production composition root;
- register all supported cluster tools;
- wire skills;
- wire memory explicitly (không default silent omission);
- wire knowledge explicitly;
- unified TenantContext;
- secret-redacted trace;
- health/readiness;
- **new `agentos/api/` Agent API entrypoint** — current implementation là 0%, legacy chat/api chỉ dùng làm migration reference;
...
- implement deterministic minimum governance decision: `RBAC ∩ ToolRisk ∩ AgentPermissionLevel`;
- acceptance: Founder+CRITICAL+L3=`ALLOW`, Member+CRITICAL+L2=`REQUIRE_APPROVAL`, Auditor+non-read=`DENY`.

**Không kéo `TenantPolicy`, `ExecutionMode`, `DataScope` vào minimum Phase 1 decision kernel.**

## Phase 2 — Governance V2 / Tool Metadata Cutover

- ToolSpecV2;
- typed `ToolRiskLevel` + `ToolPermission`;
- classify real tools bằng business review, không đoán risk từ tên;
- Executor cutover sang RBAC-aware decision kernel;
- durable approvals;
- security/audit tests;
- giữ `PermissionClass` như compatibility metadata trong transition.

`TenantPolicy`, `ExecutionMode`, `DataScope` được defer tới governance Phase 10 của implementation rollout khi minimum intersection đã ổn định.
...
## Phase 7 — Memory & Knowledge Productionization — audited scope

Phase này **không build RAG từ đầu**, nhưng cũng không phải finishing task. Chia thành 4 workstreams:

### 7A — Storage ownership

- ADR/chốt schema ownership cho `agent_memory` và `knowledge`;
- migrations thật;
- `CREATE EXTENSION vector`/vector dimension/index nếu dùng pgvector;
- upgrade/downgrade + backup/restore story.

### 7B — Memory provider architecture

- tạo explicit provider/backend boundary;
- sửa/đổi tên `PgVectorMemoryStore` nếu chưa có vector semantics;
- loại bỏ silent no-op khi session factory thiếu;
- semantic memory retrieval;
- tenant/principal isolation + retention/forget;
- optional Tencent Agent Memory adapter sau contract chuẩn.

### 7C — Knowledge productionization

- giữ lại chunk/embed/retrieve core đã có;
- parser pipeline PDF/DOCX/HTML/web/attachment;
- real pgvector migration + integration tests;
- embedding model/dimension/version metadata;
- content hash/idempotent reindex;
- provenance/citation.

### 7D — Agent integration

- wire `MemoryRetriever` + `KnowledgeRetriever` vào production composition;
- ContextBuilder/AgentContext support cả hai;
- Text Chat và Voice dùng cùng retrieval path;
- no channel-specific direct vector DB access;
- retrieval eval + tenant isolation tests.

**Verified current status:** Memory = prototype/core; Knowledge = functional core; production DB ownership/persistence/integration vẫn chưa hoàn tất.
...
- document `agentos/improvement/` as KEEP and add ownership row;
- remove any claim that `PgVectorMemoryStore` already provides vector semantic memory.

## P1 — convergence

- production AgentOS composition root;
- minimum RBAC decision kernel + acceptance tests;
- ToolSpecV2;
- governance cutover;
- TenantContext;
...
- Memory provider/backend abstraction + semantic retrieval;
- Knowledge migrations/parsers + real pgvector integration tests;
...
| Conversation/run API | TARGET `agentos/api/` (CURRENT: absent; selective legacy migration reference only) |
...
9. RBAC và Agent PermissionLevel được tách đúng; minimum `RBAC ∩ ToolRisk ∩ AgentPermissionLevel` acceptance tests pass.
10. Memory, Knowledge và Business DB không overlap ownership; Memory/Knowledge production stores fail explicitly when persistence dependencies are unavailable.
...
16. `agentos/improvement/` được document trong Ownership Map và không tự sửa/publish core capability ngoài approval/supply-chain gates.
17. `agentos/api/` có current entrypoint thật trước khi tài liệu gọi Agent API là implemented.
18. Knowledge pgvector có migration + real integration evidence trước khi gọi production-ready.
...
| Memory | InMemory store + lexical retriever + consolidation; `PgVectorMemoryStore` name nhưng không vector-search; not default-wired | MemoryService/provider architecture + durable/semantic retrieval + isolation/retention |
| Knowledge | chunk/embed/semantic-retrieve core + pgvector SQL adapter; fake-session only, no migration | migrated DB + real pgvector integration + parser/provenance + AgentContext wiring |
| Governance | PermissionClass + PermissionLevel + Agent×Risk logic; no RBAC intersection | Phase 1: RBAC × ToolRisk × PermissionLevel; later add TenantPolicy/ExecutionMode/DataScope |
| Trace | SQLite + partial telemetry | redacted SQLite checkpoint + unified OTEL |
| Agent API / Text Chat | `agentos/api/` absent; legacy workforce chat/api is migration reference only | Flutter + new Agent API + durable conversations + streaming events |
...
| Self-Improvement | `agentos/improvement/` implemented/tested, not production-eval-wired | KEEP + README/ownership + eval-history wiring + governed promotion |
...
agentos/core/embedding_provider.py
agentos/improvement/
agentos/memory/store.py
agentos/memory/pgvector_store.py
agentos/memory/retrieval.py
agentos/memory/retriever.py
agentos/knowledge/chunking.py
agentos/knowledge/ingest.py
agentos/knowledge/store.py
agentos/knowledge/retrieval.py
tests/agentos/memory/
tests/agentos/knowledge/
legacy/agent_runtime/workforce/api/
legacy/agent_runtime/workforce/chat/
...
# Appendix D — Audited Decision Register (2026-08-22)

| Audit item | Decision | Evidence/constraint |
|---|---|---|
| `agentos/improvement/` | **KEEP** | Self-improvement primitives + tests exist; missing production eval-history wiring only |
| RBAC minimum kernel | **IMPLEMENT in Phase 1** | Current `evaluate_for_agent()` has no RBAC role input; acceptance semantics are explicitly defined |
| `agentos/api/` | **GREENFIELD current surface / selectively MIGRATE behavior** | Directory absent and no current AgentOS HTTP/SSE entrypoint; legacy chat/api contains reusable streaming/event lessons |
| Memory providers | **TARGET refactor** | `agentos/memory/providers/` absent; current pgvector-named store performs no vector retrieval |
| Knowledge core | **KEEP + productionize** | chunk/embed/semantic retrieve exists; pgvector SQL exists; migration/real integration/parser/runtime wiring pending |
| Agent data DB ownership | **ADR before production cutover** | Agent memory/knowledge migrations are absent; schema ownership must be explicit |
