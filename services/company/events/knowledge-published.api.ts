 ...
 * expose:true (called cross-app over HTTP) but guarded by a worker-service JWT
 * (X-Service-Token or Authorization: Bearer), see shared/auth/worker-service-auth.ts.
 */
export const knowledgePublishedEndpoint = api(
  ...
  async (req: KnowledgePublishedRequest): Promise<{ stored: true }> => {
    return ingestKnowledgePublished({
