-- Migration 023 down: Rollback ai_governance profile backfill
...
DECLARE
  v_has_usage boolean;
BEGIN
  -- 1. Kiểm tra nếu có bất kỳ ai_governance assignment nào đã kích hoạt, không phải TEMPLATE, hoặc có metadata thực thi
  SELECT EXISTS (
    SELECT 1
    FROM operating.project_agent_assignments
    WHERE profile_key = 'ai_governance'
      AND (
        state != 'TEMPLATE'
        OR version != 1
  ...
  IF v_has_usage THEN
    RAISE EXCEPTION 'migration 023 down refused: ai_governance profile has usage evidence';
  END IF;

  -- 2. Kiểm tra nếu có sự kiện nào khác ngoài sự kiện tạo template ban đầu do migration 023 backfill
  SELECT EXISTS (
    SELECT 1
    FROM operating.project_agent_assignment_events e
    JOIN operating.project_agent_assignments a ON a.id = e.assignment_id
    WHERE a.profile_key = 'ai_governance'
      AND (
        e.event_type != 'ASSIGNMENT_TEMPLATE_CREATED'
        OR (e.event_payload->>'source') IS DISTINCT FROM 'ai_governance_profile_catalog_backfill_v1'
      )
  ) INTO v_has_usage;
  ...
  -- 4. Kiểm tra nếu có deliberation frame nào tham chiếu đến caio role
  SELECT EXISTS (
    SELECT 1
    FROM operating.project_executive_deliberation_frames f,
         jsonb_array_elements(f.selected_roles) AS r
    WHERE (r->>'roleKey') = 'caio'
  ) INTO v_has_usage;

  IF v_has_usage THEN
    RAISE EXCEPTION 'migration 023 down refused: caio role is referenced in deliberation frames';
  END IF;

  -- 5. Xoá an toàn sự kiện và assignment template của ai_governance
  DELETE FROM operating.project_agent_assignment_events
  WHERE assignment_id IN (
    SELECT id FROM operating.project_agent_assignments WHERE profile_key = 'ai_governance'
  );
