-- Migration 009 down: Rollback coding profile backfill
...
DECLARE
  v_has_usage boolean;
BEGIN
  -- 1. Kiểm tra nếu có bất kỳ coding assignment nào đã kích hoạt, không phải TEMPLATE, hoặc có metadata thực thi
  SELECT EXISTS (
    SELECT 1
    FROM operating.project_agent_assignments
    WHERE profile_key = 'coding'
      AND (
        state != 'TEMPLATE'
        OR version != 1
  ...
  IF v_has_usage THEN
    RAISE EXCEPTION 'migration 009 down refused: coding profile has usage evidence';
  END IF;

  -- 2. Kiểm tra nếu có sự kiện nào khác ngoài sự kiện tạo template ban đầu do migration 009 backfill
  SELECT EXISTS (
    SELECT 1
    FROM operating.project_agent_assignment_events e
    JOIN operating.project_agent_assignments a ON a.id = e.assignment_id
    WHERE a.profile_key = 'coding'
      AND (
        e.event_type != 'ASSIGNMENT_TEMPLATE_CREATED'
        OR (e.event_payload->>'source') IS DISTINCT FROM 'coding_profile_catalog_backfill_v1'
      )
  ) INTO v_has_usage;
  ...
  -- 4. Kiểm tra nếu có deliberation frame nào tham chiếu đến vpe role
  SELECT EXISTS (
    SELECT 1
  ...
  IF v_has_usage THEN
    RAISE EXCEPTION 'migration 009 down refused: vpe role is referenced in deliberation frames';
  END IF;

  -- 5. Xoá an toàn sự kiện và assignment template của coding
  DELETE FROM operating.project_agent_assignment_events
  WHERE assignment_id IN (
    SELECT id FROM operating.project_agent_assignments WHERE profile_key = 'coding'
  );
