...
- **Phase 1 — DB baseline promotion.** Viết Snowflake ID generator dùng chung (COSA + Company); viết migration baseline mới từ 4 file candidate, áp 5 quyết định ở 29.4; loại migration cũ khỏi fresh-bootstrap chain (giữ nguyên nội dung, bất biến); migration registry + checksum. Exit: Gate A/B/C/D (`DB_FINAL_CUTOVER.md` §13) chạy thật trên Postgres throwaway.
- **Phase 2 — Identity & tenant auth.** Xoá hardcoded default (`apps/cosa/api/routes.py:89-90,113-115,181,235`; `packages/agent_core/conversations/models.py`); auth middleware build `InvocationIdentity` (`packages/agent_core/contracts/identity.py`) tại boundary; tenant ownership check mọi query conversation/run/approval/SSE. Exit: tenant isolation test thật (E2E-5).
