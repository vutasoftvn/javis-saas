...
- **Phase 3 — Policy wiring. [~ĐÃ LÀM PHẦN LỚN, 2026-08-25]** Phát hiện quan trọng: `policy_evaluator` được `CapabilityGateway`/kernel gọi **đồng bộ** (`self._policy_evaluator(capability_id, payload, context)`, không await) — không thể tự do gọi HTTP async bên trong `evaluate()`. Giải pháp: resolve `PolicySnapshot` async TRƯỚC boundary (run-start, và lại lần nữa trước resume), nhét vào `RunRequest.metadata["policy_snapshot"]` — cũng phát hiện `state.context` trước đây bị gán nhầm `dict(request.input)` (copy prompt text, không phải context thật) khiến current-gate check cũ chưa từng match được key nào; đã sửa kernel dùng `dict(request.metadata)`.
  - **TS (services/cosa, CHƯA runtime-verify bằng Encore CLI thật — không có Docker/Encore CLI trong môi trường này):** thêm `getTenantPolicySnapshotForCaller()` (`services/agent-policy.service.ts`, tái dùng thuật toán match exact→prefix→wildcard đã có sẵn ở `getTenantPolicyForTool`) + endpoint mới `GET /platform/auth/me/agent-policy-snapshot` (`expose:true, auth:true` — khác `getTenantPolicy` cũ `expose:false` chỉ RPC nội bộ, cần endpoint public vì Python gọi qua HTTP thường). Verify caller thực sự thuộc `companyId` trước khi trả policy. `npx tsc --noEmit`: 0 lỗi ứng dụng (chỉ còn 13 lỗi nội bộ `node_modules/encore.dev` đã biết từ trước). `npx vitest run` xác nhận lại KHÔNG chạy được trong môi trường này (`ENCORE_RUNTIME_LIB` không set — không có Encore CLI) — đã viết 4 test mới trong `tests/agent-policy.test.ts` theo đúng pattern test hiện có, chưa chạy được.
  - **Python:** `apps/cosa/policies/{snapshot.py,company_policy_client.py}` (mới) — `PolicySnapshot.match()` cùng thuật toán 3 tầng; `CosaTenantPolicyClient` gọi endpoint mới. `evaluator.py::CosaPolicyEngine.evaluate()` viết lại theo đúng mô hình đã chốt ở §29.3 mục 1: current gate từ snapshot (DENY nếu company/principal không "active") → `emergency_lock` → tenant override (`snapshot.match()`) → rule hardcode cũ làm fallback (giữ nguyên, không xoá). `apps/cosa/composition/agent_plane.py` thêm field `tenant_policy_client`. `apps/cosa/api/routes.py`: resolve snapshot trước `kernel.run()` (fail run rõ ràng nếu resolve lỗi — không ALLOW ngầm, đúng §10.5) và lại lần nữa trước `kernel.resume()` (current gate re-observe tại thời điểm resume, không dùng lại snapshot pin từ run-start).
  - **Test thật:** 296 test pass Python (279 + 17 mới cho `tests/apps/cosa/policies/`: `PolicySnapshot.match()`, `CosaTenantPolicyClient` qua `httpx.MockTransport`, `CosaPolicyEngine.evaluate()` với snapshot — current gate suspended/revoked/emergency_lock đều DENY đúng, tenant override ALLOW/DENY thắng rule hardcode, fallback hardcode khi không có rule tenant, tương thích ngược context cũ không qua snapshot); 2 vertical-slice test cũ + tenant-isolation test cập nhật mock `tenant_policy_client` (trước đây thiếu mock sẽ fail-closed đúng như thiết kế — xác nhận cơ chế fail-closed hoạt động thật, không phải lý thuyết). Không regression (`tests/agent_core` không đổi kết quả).
  - **Còn thiếu:** benchmark/tối ưu tần suất gọi snapshot (hiện 1 lần/run-start + 1 lần/resume, không phải mỗi tool call — đủ cho 2 boundary async hiện có của hệ thống, nhưng chưa có cơ chế "re-check giữa chừng 1 run dài chạy nhiều tool call" nếu sau này cần); TS endpoint mới chưa chạy `encore test` thật.
- **Phase 4 — Durable dispatch/worker/lease. [~ĐÃ LÀM PHẦN LỚN, 2026-08-25]** Đã xoá hoàn toàn `asyncio.create_task` khỏi `apps/cosa/api/routes.py` (`create_message` và `decide_approval`) — thay bằng `await plane.scheduler.schedule(...)` (durable, persist trước khi return `202`/`200`). Logic thực thi run/resume (trước đây `_execute_canonical_run_task`/`do_resume` sống trong hàm HTTP handler) đã MOVE nguyên vẹn sang `apps/cosa/worker/handlers.py::execute_run_task`/`execute_resume_task` — dùng chung bởi worker process mới `apps/cosa/worker/main.py` (entrypoint `python -m apps.cosa.worker.main`, target Makefile `agent-worker`).
  - **Scheduler HTTP client mới:** `packages/agent_core/coordination/control_plane_scheduler_client.py::HttpControlPlaneSchedulerClient` (chưa tồn tại trước phiên này — chỉ có `HttpControlPlaneLeaseClient` sẵn) — cùng interface `RunScheduler` (`schedule`/`poll_due_tasks`/`complete_task`), gọi 3 endpoint `services/cosa/handlers/control-plane.handler.ts` đã có sẵn từ Wave 7 (`scheduleTaskEndpoint`/`pollDueScheduledTasksEndpoint`/`completeScheduledTaskEndpoint`) — chỉ thiếu client Python, không thiếu hạ tầng TS.
  - **Wire làm default thật (không chỉ "tồn tại sẵn chưa ai dùng" như trước phiên này):** `apps/cosa/composition/agent_plane.py::build_cosa_agent_plane()` mặc định dùng `HttpControlPlaneSchedulerClient`/`HttpControlPlaneLeaseClient` (gọi `COSA_CONTROL_PLANE_URL`, mặc định `http://127.0.0.1:4001` khớp `make services-dev-cosa`) — KHÔNG âm thầm rơi về `RunScheduler`/`RunLeaseManager` in-memory, cùng nguyên tắc "production không silent fallback" đã áp dụng cho repository/conversation_repository/governance_store. Test/dev truyền tường minh `scheduler=RunScheduler()`, `lease_client=RunLeaseManager()`.
  - **Worker loop** (`run_worker_loop`/`dispatch_one_task`): poll → atomic claim (đã có sẵn ở tầng scheduler, `FOR UPDATE SKIP LOCKED` phía TS) → acquire lease cho `run_id` → dispatch theo `task_type` (`run`/`resume`) → **renew lease heartbeat song song trong lúc execute** (đúng flow §5.2 gốc, không phải chỉ acquire-rồi-quên) → release lease → `complete_task`. Nếu acquire lease thất bại (worker khác đang giữ), KHÔNG complete_task — để nguyên cho worker giữ lease xử lý.
  - **Phát hiện thật khi viết test (đáng chú ý):** lần đầu viết test mô phỏng "2 worker" bằng cách gọi `dispatch_one_task` 2 lần trong CÙNG 1 test process — test FAIL vì cả 2 lần đều acquire lease thành công! Nguyên nhân: `WORKER_ID` là hằng số module-level (đúng thiết kế — 1 process = 1 worker_id cố định), nên "2 lần gọi cùng process" thực chất là CÙNG 1 worker_id, và `RunLeaseManager` cho phép cùng worker_id "re-acquire" lease của chính nó (đúng, cần thiết cho idempotent retry). Đã sửa lại test dùng 2 worker_id tường minh khác nhau gọi thẳng `lease_client`. Bản thân việc bug-test này xảy ra là minh chứng sống cho đúng điều CLAUDE.md #6 cảnh báo — test cùng-process dễ tạo ảo giác đã chứng minh cross-process behavior trong khi chưa hề.
  - **Test thật:** 293 test Python pass (288 + 5 mới `tests/apps/cosa/worker/test_main.py` + 4 mới `tests/agent_core/coordination/test_control_plane_scheduler_client.py`) — 2 vertical-slice test cũ + tenant-isolation test đổi từ `await asyncio.sleep(0.1)` (chờ hên xui) sang `await drain_worker_queue(plane)` (dispatch tường minh, xác định số task đã xử lý bằng assert, không phải suy đoán qua sleep).
  - **CHƯA làm — đây là gap thật, KHÔNG được coi Phase 4 "done":**
    1. **Cross-process crash-recovery thật (CLAUDE.md #6) — CHƯA chứng minh được trong môi trường này.** Đã thử nghiêm túc: dựng `@electric-sql/pglite-socket` (Postgres wire-protocol server thật chạy trên WASM engine, không cần Docker) để cho phép 2 process Python thật (subprocess) cùng nói chuyện với 1 "Postgres" — `asyncpg` kết nối treo vô thời hạn (không lỗi rõ ràng, nghi ngờ pglite-socket chưa tương thích đầy đủ handshake của asyncpg), đã huỷ thử nghiệm sau khi xác nhận treo thật (không phải chờ chưa đủ lâu). Cần Docker Postgres thật (CI/staging) để làm test này đúng cách — kill worker process thật giữa run, worker process thật khác resume, verify qua DB thật.
    2. **Stuck-task sweeper chưa có:** nếu worker chết giữa chừng SAU khi đã claim task (status="processing") nhưng TRƯỚC khi complete, task đó kẹt "processing" mãi — không có cơ chế quét lại task "processing" có lease đã hết hạn để reset về "scheduled". Lease tự nó bảo vệ đúng run_id khỏi 2 worker chạy song song, nhưng không tự động "hồi sinh" task bị bỏ dở.
    3. Deployment (docker-compose service cho `agent-worker`, nhiều replica) — Phase 8.
- **Phase 5 — Durable SSE. [~ĐÃ LÀM PHẦN LỚN, 2026-08-25]** `apps/cosa/api/event_stream.py::CosaEventStreamManager` viết lại hoàn toàn: `_history` dict đã xoá, `emit()`/`stream_events()` giờ nhận `repository` (`RunStreamEventRepository`) làm tham số — persist NGAY qua `repository.append()` trước khi fanout; replay đọc `repository.list_since(run_id, after_sequence=...)`, không phải RAM của instance hiện tại. `_queues` chỉ còn live-fanout optimization. Heartbeat: không đóng stream khi im lặng (trước đây đóng sau 2s) — gửi `: heartbeat\n\n` mỗi 15s, chỉ đóng khi có terminal event (`run.completed`/`run.failed`/`run.cancelled`) hoặc client tự ngắt.
  - **Phản biện so với §7.2 gốc — KHÔNG dùng chung `agent_core.run_events`.** Phát hiện khi implement: `OpenAIAgentsKernel` đã tự ghi vào `agent_core.run_events` với vocabulary generic riêng (`run.started`, `run.completed{final_output}`, `run.failed`, `tool.started`, `tool.completed`, `run.waiting`, `run.resumed` — xem `_emit_event()` trong kernel) — đã chạy production TỪ TRƯỚC, không phải phát hiện mới. Vocabulary này TRÙNG TÊN nhưng KHÁC payload shape với vocabulary app-facing hiện có (`run.started`, `reasoning.status`, `message.started`, `message.delta`, `run.completed{output,status}`, `approval.required`, `approval.resolved`, `run.failed`, `run.cancelled`). Ghi chung 1 bảng sẽ tạo event trùng/xung đột khi replay (2 "run.completed" khác payload cho cùng run). Đổi vocabulary/payload app-facing để tránh trùng là đổi hợp đồng SSE với Flutter frontend — không thể verify an toàn trong phiên này (không có môi trường chạy Flutter/browser). Quyết định: bảng mới `agent_conversation.run_stream_events` (migration `011_run_stream_events.sql`, cùng schema `agent_conversation` đã có từ migration 006) — giữ nguyên 100% event vocabulary/payload hiện tại của frontend, chỉ đổi persistence layer từ RAM sang Postgres. Đây không phải "nhân bản kiến trúc" (CLAUDE.md #4): `agent_core.run_events` = kernel governance/audit ledger; `run_stream_events` = app UX fanout log — 2 concern khác nhau, ownership khác nhau (agent_core vs apps/cosa).
  - **Model/repository mới:** `packages/agent_core/runs/stream_events.py::RunStreamEventRecord`/`RunStreamEventRepository`/`InMemoryRunStreamEventRepository`/`PostgresRunStreamEventRepository` — đặt ở `packages/agent_core` (không phải `apps/cosa`) vì đây là generic "run stream event" concept, cùng pattern với `agent_core/conversations/`. `sequence` dùng `BIGSERIAL` toàn cục (không reset theo run_id) — cùng pattern đã proven ở `agent_core.run_events.sequence_no`, tránh hoàn toàn race condition tính `MAX(sequence)+1` dưới concurrent insert.
  - **Migration `011_run_stream_events.sql`** — verify thật qua `@electric-sql/pglite` (WASM Postgres, cùng công cụ dùng ở Phase 1): áp dụng sạch trên top của migration 001-008 (bỏ qua 003/009/010 vì cần extension `pgvector` không có trong bản pglite cơ bản, không liên quan thay đổi này), `UNIQUE` constraint qua PK `sequence` hoạt động đúng.
  - **`build_cosa_agent_plane()`** thêm `stream_event_repository` theo ĐÚNG pattern "production không silent fallback in-memory" đã áp dụng cho repository/conversation_repository/spec_registry/governance_store — `RuntimeError` nếu thiếu cả tham số tường minh lẫn `AGENT_CORE_DATABASE_URL`.
  - **Phát hiện thật khi viết test (đáng chú ý):** test đầu tiên cho case "SSE stream giữa chừng chưa có terminal event" bị TREO vô thời hạn khi chạy qua `httpx.ASGITransport` + `.text` — vì thiết kế heartbeat mới ĐÚNG Ý ĐỊNH (không đóng stream khi im lặng) nhưng `httpx.ASGITransport` trong môi trường test **không hỗ trợ true incremental streaming**: nó buffer toàn bộ response tới khi generator kết thúc hẳn mới trả chunk đầu tiên (xác nhận bằng thực nghiệm riêng, không phải suy đoán — xem `test_vertical_slice_2_write_approval.py` commit history). Với generator không có điểm kết (đợi terminal event mãi mãi), điều này khiến MỌI HTTP-level test cho case "stream chưa terminal" treo vô thời hạn — không phải bug ở code SSE, mà là giới hạn thật của công cụ test. Đã sửa: case mid-flight (chưa terminal) verify trực tiếp qua `repository.list_since()` thay vì qua HTTP; case terminal (có `run.completed`) vẫn verify qua HTTP vì ASGITransport hoạt động bình thường khi generator tự kết thúc.
  - **Test thật:** 299 test Python pass (293 + 6 mới `tests/apps/cosa/test_event_stream.py`, gồm test chứng minh cốt lõi: instance `CosaEventStreamManager` MỚI — mô phỏng "API process restart" — vẫn replay đúng lịch sử vì nguồn sự thật là repository chứ không phải RAM của instance cũ).
  - **CHƯA làm:** E2E-4 thật (reconnect `Last-Event-ID` sau khi API process THẬT restart, qua HTTP thật) — cần môi trường có thể chạy uvicorn/gunicorn thật + Postgres thật, không có trong phiên này; SSE payload từ kernel-native events (`tool.started`/`tool.completed`/`run.waiting`) chưa được expose cho frontend dù đã durable sẵn trong `agent_core.run_events` — cơ hội làm giàu UX sau nếu cần, không phải gap chặn.
- **Phase 6 — Control-plane consumer verify thật. [KHÔNG LÀM ĐƯỢC TRONG PHIÊN NÀY — thiếu hạ tầng]** Môi trường phiên 2026-08-25 không có Docker/Encore CLI (đã xác nhận: `which encore` → not found), nên không thể chạy `encore run`/`encore test` thật — đây là giới hạn hạ tầng thật, không phải bỏ qua. Đã làm phần tĩnh có thể làm: `npx tsc --noEmit` phía `services/cosa` vẫn sạch (0 lỗi ứng dụng) sau toàn bộ thay đổi Phase 1-5; cập nhật lại các comment "KHÔNG có consumer production" trong `control-plane.handler.ts`/`control-plane-schema.ts` cho chính xác theo trạng thái mới — `leases`/`scheduled-tasks` endpoint giờ CÓ consumer production thật (`apps/cosa/worker/main.py`, wired làm default từ Phase 4), `missions/tasks/workers/watches/delivery` vẫn chưa; cập nhật `ADR-CONTROLPLANE-001` header (implementation đã bắt đầu, không còn "chưa bắt đầu"). **Việc bắt buộc cho CI/staging trước khi coi Phase 6 xong:** chạy `encore run` local + Postgres thật, integration test worker Python → endpoint Encore thật (không mock), benchmark latency đo lại thật (không dùng số liệu tường thuật session cũ trong `COSA_AGENT_PLATFORM_BLUEPRINT_V2_RECONCILED_PLAN_2026-08-24.md` — số đó không tái tạo được trong môi trường này).
- **Phase 7 — Runtime hardening (theo ADR-RUNTIME-002). [QUYẾT ĐỊNH ĐÃ CHỐT, TRIỂN KHAI BLOCKED bởi thiếu DeepSeek API key]** Comment lỗi thời trong `agent_plane.py` đã sửa từ Phase 3 (trỏ đúng ADR-RUNTIME-002 thay vì "ADR-RUNTIME-001 DRAFT chưa review"). **`AdkCofounderWorkflow` — đã hỏi người dùng: RETIRE** (không port sang canonical, xóa cùng đợt dọn `legacy/` ở Phase 10, không cần characterization test parity vì không giữ hành vi). Đây là quyết định độc lập với ADR-RUNTIME-002 (chọn kernel generic) — đã đóng dứt điểm, không mở lại.
  - **Harden DeepSeek conformance/checkpoint-resume/parallel-tool-calls/restart-recovery: KHÔNG làm được trong phiên này** — môi trường không có `DEEPSEEK_API_KEY`/`OPENAI_API_KEY` nào (đã kiểm tra `env`, không có), không thể gọi model thật để verify conformance suite. Test đã tồn tại từ trước (`packages/agent_testkit/kernel_conformance/test_openai_agents_sdk_kernel_deepseek_live.py`, theo báo cáo phiên trước) — CHƯA re-verify được trong phiên này, không nên coi báo cáo cũ là bằng chứng hiện tại (đúng nguyên tắc §29.1-29.2 của addendum này — tường thuật session cũ không tái tạo được ≠ đã xác nhận).
  - **Việc cần làm ở CI/staging có API key thật:** chạy lại `agent_testkit/kernel_conformance/` đầy đủ cho `OpenAIAgentsKernel`, xác nhận restart-recovery thật (kill process giữa checkpoint, resume ở process khác — cùng giới hạn cross-process đã ghi ở Phase 4).
- **Phase 8 — Legacy deployment convergence. [~ĐÃ LÀM PHẦN AN TOÀN, PHẦN RỦI RO CAO CHƯA LÀM — cần xác nhận riêng]** Đã mở rộng `boundary-check` (`tests/apps/cosa/test_services_boundary_audit.py::test_deployment_configs_legacy_references_are_allowlisted`, tự động chạy trong `make boundary-check` vì target đã chạy cả file): quét `docker-compose*.yml`/`Dockerfile*`/`Makefile` (loại trừ `node_modules`/`legacy`/`.git`/`agent_runtime_archive`) cho path `legacy/`, so với allowlist tường minh theo SỐ LƯỢNG reference đã ghi nhận (`Makefile`: 2, `docker-compose.yml`: 22) — fail nếu có file mới/không ghi nhận, hoặc số lượng thay đổi (ai đó thêm/bớt dependency mà không cập nhật allowlist). Đã tự kiểm chứng thật: thêm 1 dòng `legacy/` giả vào `services/docker-compose.yml`, test fail đúng như kỳ vọng, rồi revert.
  - **CHƯA làm — cố ý, vì rủi ro cao và không thể verify an toàn trong phiên này:** xoá mount `legacy/backend` khỏi 4 service (`migrate`, `migrate-control-plane`, `brain-api`, `agent-worker`) trong `docker-compose.yml`. Lý do dừng lại: (1) `make deploy-control-plane` (target production thật) hiện ĐANG PHỤ THUỘC `migrate-control-plane` (Alembic, legacy) để migrate schema `cosa_control_plane` — `services/cosa/scripts/migrate.mjs` đọc CÙNG env var `CONTROL_PLANE_DATABASE_URL`, gợi ý ý định thay thế đúng, nhưng KHÔNG có cách nào trong môi trường này (không Docker/Postgres) để xác nhận schema/dữ liệu khớp nhau trước khi cắt; (2) đây là thay đổi ảnh hưởng trực tiếp production deployment thật (VPS) — đúng loại hành động CLAUDE.md mục "Hành động rủi ro cao... cần approval qua code" và an toàn sửa code mục #10 ("không tự ý xóa... xác nhận với người dùng trước hành động phá hủy") — không tự ý làm dù có thể về mặt kỹ thuật.
  - **Việc cần làm ở CI/staging trước khi cắt:** xác nhận `node scripts/migrate.mjs` (services/cosa) tạo ra đúng schema `cosa`/`control_plane` tương đương những gì `migrate-control-plane` (Alembic) từng tạo trên môi trường đang chạy thật; thêm service `migrate-cosa`/`migrate-company`/`agent-worker` (Python, `apps.cosa.worker.main`) MỚI vào `docker-compose.yml` trước, chạy song song để kiểm tra tương đương, rồi mới xoá 4 service cũ trong 1 thay đổi riêng có sign-off rõ ràng.
- **Phase 9 — CI/E2E gate đầy đủ. [~ĐÃ LÀM PHẦN AN TOÀN/VERIFY-ĐƯỢC, phần cần Docker orchestration CHƯA làm]** Thêm job `apps-cosa` mới vào `.github/workflows/quality.yml` — trước đây `tests/apps/cosa/` chỉ chạy 2 test boundary-check qua job `boundaries`, TOÀN BỘ test Phase 2-5 (auth/policies/worker/event_stream/vertical-slice/tenant-isolation, ~50 test mới trong phiên này) chưa từng chạy CI (đúng gap §19.2 tài liệu gốc: "chỉ tồn tại trong repo").
  - **Phát hiện + sửa 1 bug thật trong lúc thêm job:** cả job `agent-core` (đã tồn tại từ trước) lẫn job `apps-cosa` mới đều cần `pytest`/`pytest-asyncio` (bắt buộc vì `pytest.ini` khai `asyncio_mode = strict`) nhưng KHÔNG package nào trong `packages/agent_core/requirements.txt` hay `apps/cosa/requirements.txt` khai 2 gói này (có chủ đích — không phải runtime dependency của agent_core/apps.cosa) — job `agent-core` cũ thiếu bước cài đặt này, `pip install -r packages/agent_core/requirements.txt` một mình sẽ khiến bước `pytest ...` fail ngay "command not found". Đã sửa CẢ 2 job (thêm `pytest pytest-asyncio` vào lệnh `pip install`, cùng pattern `realtime-agent` job đã dùng cho `pytest>=8.0.0`).
  - **Verify:** YAML parse hợp lệ (`python -c "import yaml; yaml.safe_load(...)"`), quét tĩnh toàn bộ import trong `apps/cosa/`+`tests/apps/cosa/` xác nhận không thiếu dependency nào khác ngoài 2 file requirements + pytest/pytest-asyncio. **CHƯA chạy job này trên GitHub Actions thật** (không thể trigger CI từ phiên này) — verify tĩnh, không phải chạy thật.
  - **CHƯA làm — cố ý, vì không thể verify an toàn trong môi trường này:** job `migration-baseline` (fresh-bootstrap Gate A/B/C/D riêng biệt — job `services` hiện tại đã ngầm làm một phần Gate A qua `node scripts/migrate.mjs` mỗi lần chạy trên Postgres rỗng, nhưng chưa tách thành gate độc lập có report riêng), `full-stack-golden-path`, `restart-recovery` (cần orchestration nhiều service + worker + Encore đồng thời qua Docker Compose thật — viết YAML cho việc này mà không chạy thử được có rủi ro cao hơn giá trị mang lại, dễ tạo "CI xanh giả" nếu có lỗi cú pháp/logic không phát hiện được). E2E-1 đến E2E-7 (§20) vẫn chưa có bất kỳ job nào — để lại cho CI/staging có khả năng chạy thử Docker Compose thật trước khi viết.
