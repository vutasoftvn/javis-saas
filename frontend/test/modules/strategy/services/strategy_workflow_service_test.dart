...
void main() {
  ...
  group('StrategyWorkflowService', () {
    test('getWorkspaceSettings sends GET /operations/strategy/settings with headers', () async {
    ...
    test('updateWorkspaceSettings maps 409 conflict to RevisionConflictException', () async {
      ApiClient.client = MockClient((request) async {
        return http.Response(
          jsonEncode({
      ...
      expect(
        ...
        throwsA(isA<RevisionConflictException>().having(
          (e) => e.message,
          'message',
          contains('conflict'),
        )),
    ...
    test('listStrategicObjectives, PESTEL, SWOT and TOWS request paths', () async {
      final paths = <String>[];
      ...
      ApiClient.client = MockClient((request) async {
        ...
        if (request.url.path == '/operations/strategy/objectives/obj_1/analysis/pestel') {
          return http.Response(
            jsonEncode({
              'items': [
                {
        ...
        if (request.url.path == '/operations/strategy/objectives/obj_1/analysis/swot') {
          return http.Response(
            jsonEncode({
              'items': [
                {
        ...
        if (request.url.path == '/operations/strategy/objectives/obj_1/tows-options') {
          return http.Response(
            jsonEncode({
              'items': [
                {
      ...
      expect(paths, contains('/operations/strategy/objectives/obj_1/analysis/pestel'));
      expect(paths, contains('/operations/strategy/objectives/obj_1/analysis/swot'));
      expect(paths, contains('/operations/strategy/objectives/obj_1/tows-options'));
    });
    ...
    test('never emits requests to ghost routes starting with /strategy/lenses', () async {
      ...
      final lensService = StrategyLensService(workflowService: workflowService);
      ...
      for (final p in interceptedPaths) {
        expect(p.startsWith('/strategy/lenses'), isFalse,
