...
void main() {
  ...
  setUp(() async {
    ...
    await SecureStorageService.write('workspace_id', 'ws-123');
  ...
  group('Task 11: Initiative Planning, Reviews Timeline & Settings Tests', () {
    testWidgets(
      '1. Initiative approval state gates task association until approved',
      (tester) async {
        // Setup mock backend with 1 DRAFT initiative
        Map<String, dynamic>? createdTaskBody;
        ApiClient.client = MockClient((request) async {
          ...
          if (request.url.path == '/operations/initiatives/init-1/approve' &&
              request.method == 'POST') {
            return http.Response(
              jsonEncode({
                'id': 'init-1',
          ...
          if (request.url.path == '/operations/tasks' &&
              request.method == 'POST') {
            createdTaskBody = jsonDecode(request.body) as Map<String, dynamic>;
        ...
        await tester.tap(
          find.byKey(const ValueKey('task_initiative_dropdown')),
        );
        await tester.pumpAndSettle();
        ...
        expect(
          find.byKey(const ValueKey('task_assignment_error_message')),
          findsOneWidget,
        );
        expect(
          find.textContaining('chưa được phê duyệt (Approval: DRAFT)'),
          findsOneWidget,
        );

        // Now approve the initiative
        final approveBtn = find.byKey(
          const ValueKey('approve_initiative_button_init-1'),
        );
        ...
        expect(createdTaskBody, {
          'workspaceId': 'ws-123',
          'title': 'Soạn hợp đồng đại lý mẫu',
          'initiativeId': 'init-1',
        });
      },
    );
    ...
    testWidgets(
      '2. BAU task creation allows unlinked tasks without initiative',
      (tester) async {
        Map<String, dynamic>? createdTaskBody;
        ApiClient.client = MockClient((request) async {
          if (request.url.path == '/operations/tasks' &&
              request.method == 'POST') {
            createdTaskBody = jsonDecode(request.body) as Map<String, dynamic>;
            return http.Response(jsonEncode({'id': 'task-bau-1'}), 200);
          }
          if (request.url.path == '/operations/initiatives') {
            return http.Response(
        ...
        await _pumpApp(
          tester,
          const InitiativePlanPanel(strategicObjectiveId: 'strat-1'),
        );
        ...
        expect(createdTaskBody, {
          'workspaceId': 'ws-123',
          'title': 'Họp giao ban định kỳ thứ 2',
        });
      },
    );

    testWidgets(
      '3. One- and two-option policies are configured and constrained',
      (tester) async {
        ...
        await _pumpApp(tester, const WorkspaceStrategySettingsSheet());

        // Verify initial limit is 1 option
        expect(
          find.text('1 chiến lược trọng tâm (Tập trung tối đa)'),
          findsOneWidget,
        );

        // Change limit to 2 options
        await tester.tap(
          find.byKey(const ValueKey('settings_tows_limit_dropdown')),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      '4. 10-week cycle positions Mid-cycle review at Week 5 with server permission gating',
      (tester) async {
        final reviews = [
          {
            'id': 'rev-w1',
        ...
        expect(
          find.byKey(const ValueKey('review_week_rev-mid5')),
          findsOneWidget,
        );
        expect(find.text('Tuần 5'), findsOneWidget);
        expect(
          find.byKey(const ValueKey('review_decision_link_rev-mid5')),
          findsOneWidget,
        );
        ...
        final closeBtn = find.byKey(
          const ValueKey('close_review_button_rev-mid5'),
        );
        ...
        // Confirm close
        await tester.tap(
          find.byKey(const ValueKey('confirm_close_review_button')),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      '5. Settings sheet enters read-only state when canEdit is false',
      (tester) async {
        ApiClient.client = MockClient((request) async {
          if (request.url.path == '/operations/strategy/settings') {
            return http.Response(
        ...
        expect(
          find.byKey(const ValueKey('settings_read_only_banner')),
          findsOneWidget,
        );
        expect(
          find.textContaining('Chế độ chỉ đọc: Bạn không có quyền'),
          findsOneWidget,
        );
    ...
    testWidgets(
      '6. Settings sheet recovers from 409 revision conflict by reloading',
      (tester) async {
        int getCalls = 0;
          ...
          if (request.url.path == '/operations/strategy/settings') {
            ...
            if (request.method == 'PUT') {
              ...
              return http.Response(
                jsonEncode({
                  'message':
                      'Settings revision conflict. Expected 1 but was 2.',
                  'currentRevision': 2,
                }),
                409,
                headers: {'content-type': 'application/json; charset=utf-8'},
        ...
        expect(getCalls, 2);
        expect(
          find.byKey(const ValueKey('settings_conflict_banner')),
          findsOneWidget,
        );
