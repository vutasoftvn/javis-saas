  ...
  group('AgentPlatformService Tests', () {
    test('getAgents returns list of agents', () async {
      ApiClient.client = MockClient((request) async {
        expect(request.url.path, '/api/v1/agent-platform/agents');
    ...
    test('getTools returns list of tools', () async {
      ApiClient.client = MockClient((request) async {
        expect(request.url.path, '/api/v1/agent-platform/tools');
        return http.Response(
          jsonEncode([
    ...
    test('testRouting returns classified intent', () async {
      ApiClient.client = MockClient((request) async {
        expect(request.url.path, '/api/v1/agent-platform/routing/test');
