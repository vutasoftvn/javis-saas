...
/// Lộ trình nâng cao: MVP roadmap (tạo/sửa thủ công hoặc AI)
/// -> founder sửa & xác nhận -> chọn 1 stage để lập kế hoạch & kích hoạt.
class ProjectRoadmapAdvancedView extends StatefulWidget {
  final String projectId;
  final VoidCallback onBack;
  final void Function(String stageId) onOpenStageWorkspace;

  const ProjectRoadmapAdvancedView({
    super.key,
  ...
  @override
  State<ProjectRoadmapAdvancedView> createState() =>
      _ProjectRoadmapAdvancedViewState();
}

class _ProjectRoadmapAdvancedViewState
    extends State<ProjectRoadmapAdvancedView> {
  ProjectOrchestrationController get controller =>
  ...
  @override
  void didUpdateWidget(covariant ProjectRoadmapAdvancedView oldWidget) {
                ...
                Row(
                  ...
                  children: [
                    Text(
                      isDraftOpen
                          ? (hasConfirmed
                                ? 'Chỉnh sửa Lộ trình MVP (Bản nháp)'
                                : 'Lộ trình MVP (Bản nháp)')
                          : (hasConfirmed
                                ? 'Lộ trình MVP (Đã xác nhận)'
                                : 'Lộ trình MVP (Chưa khởi tạo)'),
                  ...
                  children: [
                    ...
                    OutlinedButton.icon(
                      ...
                      label: Text(
                        isDraftOpen
                            ? '+ Thêm giai đoạn'
                            : (hasConfirmed ? 'Sửa thủ công' : 'Tạo thủ công'),
                      ),
                    ...
                    OutlinedButton.icon(
                      onPressed: controller.isGeneratingRoadmap.value
                          ? null
                          : () => _showAiPromptDialog(context),
                      icon: const Icon(
                    ...
                    ElevatedButton.icon(
                      onPressed: controller.isGeneratingRoadmap.value
                          ? null
                          : () => controller.generateRoadmap(widget.projectId),
                      ...
                      ElevatedButton.icon(
                        onPressed: controller.isSaving.value
                            ...
                            : () async {
                                ...
                                if (saved) {
                                  await controller.confirmRoadmap(
                                    widget.projectId,
                                  );
                                }
                              },
              ...
              children: [
                ...
                if (index > 0)
                  IconButton(
                    ...
                    color: AppTheme.textMutedDark,
                    onPressed: () =>
                        controller.moveDraftStage(index, index - 1),
                  ),
                ...
                IconButton(
                  ...
                  color: AppTheme.primary,
                  onPressed: () =>
                      _showStageEditDialog(context, stage, stageIndex: index),
                ),
                IconButton(
        ...
        child: Column(
          ...
          children: [
            ...
            for (final stage in controller.stages)
              _confirmedStageRow(stage as Map<String, dynamic>),
          ],
        ),
      );
    });
              ...
              return Padding(
                ...
                child: ElevatedButton(
                  onPressed: controller.isSaving.value
                      ? null
                      : () =>
                            controller.activateStage(widget.projectId, stageId),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppTheme.success,
                    foregroundColor: Colors.white,
                  ),
  ...
  void _showStageEditDialog(
    ...
    Map<String, dynamic>? initialStage, {
    int? stageIndex,
    bool isNew = false,
  }) {
    final titleCtrl = TextEditingController(
      text: initialStage?['title']?.toString() ?? '',
    );
    final hypothesisCtrl = TextEditingController(
      text: initialStage?['hypothesis']?.toString() ?? '',
    );

    final scopeItems = List<String>.from(
      (initialStage?['scope'] as List<dynamic>?)?.map((e) => e.toString()) ??
          [],
    ).obs;
    final nonGoalsItems = List<String>.from(
      (initialStage?['non_goals'] as List<dynamic>?)?.map(
            (e) => e.toString(),
          ) ??
          [],
    ).obs;
    final exitCriteriaItems = List<String>.from(
      (initialStage?['exit_criteria'] as List<dynamic>?)?.map(
            (e) => e.toString(),
          ) ??
          [],
    ).obs;
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Ví dụ: Phát triển nguyên mẫu đăng nhập và onboarding',
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Mô tả giả thuyết cốt lõi cần kiểm chứng trong giai đoạn này...',
                    hintStyle: const TextStyle(
                      color: AppTheme.textMutedDark,
                      fontSize: 13,
                    ),
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText:
                      'Ví dụ: Hãy chia lộ trình thành 2 giai đoạn chính, giai đoạn 1 tập trung hoàn toàn vào xác thực người dùng và thanh toán...',
                  hintStyle: const TextStyle(
                    color: AppTheme.textMutedDark,
                    fontSize: 13,
                  ),
