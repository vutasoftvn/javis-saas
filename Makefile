...
ts-suppression-check:
	PYTHONPATH=$(CURDIR) $(PYTEST) tests/quality/test_ts_suppressions.py -q
	node scripts/check_ts_suppressions.mjs --root . --baseline scripts/ts-suppression-baseline.json

frontend-api-contract-check:
	# Task 7 — chặn literal route trong frontend/lib lệch khỏi
	# shared/contracts/mvp-surface.json (route đã bị xoá không được lặng lẽ quay
	# lại, route unknown không được gọi thẳng bằng string tay).
	PYTHONPATH=$(CURDIR) $(PYTEST) tests/quality/test_frontend_api_contracts.py -q
...
route-auth-allowlist-check:
	PYTHONPATH=$(CURDIR) $(PYTEST) tests/quality/test_route_auth_allowlist.py -q
...
local-knowledge-e2e: ## Task 13 (plan local-first-enterprise-knowledge): restart durability + workspace isolation for Vault/knowledge ingestion — 4 real planes + disposable PG, needs Encore CLI + PGPASSWORD (see docs/operations/local-knowledge-runbook.md)
	mkdir -p test-results
	PYTHONPATH=. $(PYTEST) tests/e2e/test_local_knowledge_workspace.py -m cross_plane -q --junitxml=test-results/e2e-local-knowledge.xml

lease-integration-test: ## Task 7 (plan platform-authority-durability-hardening): lease mutual-exclusion + crash-recovery durability — 1 disposable Postgres cluster + services/cosa Encore thật per test (không port cố định, không fallback DATABASE_URL), needs Encore CLI + PGPASSWORD khớp POSTGRES_PASSWORD thật trong .env (KHÔNG phải mặc định "postgres" của thư viện test)
	mkdir -p test-results
	LEASE_INTEGRATION_TEST=1 PYTHONPATH=. $(PYTEST) tests/apps/cosa/worker/test_lease_mutual_exclusion_real.py tests/apps/cosa/worker/test_crash_recovery_subprocess.py -m durability -q --junitxml=test-results/lease-integration.xml
...
test-db-reset: ## Recreate ONLY the three Founder Trial test databases (test-only, fail-closed)
...
# I-3 (P4-P5 final review, 2026-09-02) — variant của `dev-stack` KHÔNG chạy
# `dev-infra` (docker compose) hay `dev-migrate`. Dùng cho runner không có
# Docker (vd. job `frontend-integration-real` trên `macos-latest` — GitHub
# Actions KHÔNG hỗ trợ docker daemon trên macOS runner) mà Postgres/migration
# đã được chuẩn bị sẵn bằng đường khác (Homebrew Postgres + `node
# scripts/migrate.mjs` chạy như các step CI riêng trước đó).
