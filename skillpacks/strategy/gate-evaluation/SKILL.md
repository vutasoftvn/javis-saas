...
- Có `projectId`, `currentStage` và `targetStage`.
...
2. **Gọi tool tạo Gate Evaluation**: Gọi `strategy.gate_evaluation.create` với `projectId`, `currentStage`, `targetStage`, `passed`, `score`, `notes`.
3. **Diễn giải kết quả**: Trình bày rõ ràng lý do đạt hoặc chưa đạt dựa trên phản hồi của tool và stage policy.
...
- **Dự Án**: [ID / Tên dự án]
- **Chuyển Giai Đoạn**: [Current Stage] -> [Target Stage]
- **Kết Quả**: [PASSED / FAILED]
- **Điểm Đạt Được**: [Score / Điểm số]
- **Chi Tiết Tiêu Chí**:
  - [Tiêu chí 1: Đạt / Chưa đạt]
...
- **Execution**: Gọi `strategy.gate_evaluation.create(projectId=..., currentStage='S1_DISCOVERY', targetStage='S2_VALIDATION', ...)` -> Trả về kết quả đánh giá từ backend.
