...
`services/cosa/services/control-plane-delivery.service.ts` (Wave 7, TypeScript/Encore) — delivery là control-plane primitive, thuộc `services/cosa`, KHÔNG thuộc `packages/agent_core` (theo `ADR-CONTROLPLANE-001-control-plane-primitives-in-services-cosa.md`).
...
- Không gọi API gửi tin nhắn trực tiếp từ `packages/agent_core` — mọi delivery side effect qua `services/cosa` control-plane, giữ đúng ranh giới CLAUDE.md ("Agent Platform không tự quyết định... ghi business DB trực tiếp").
