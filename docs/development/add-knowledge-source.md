...
1. Đọc `packages/agent_core/knowledge/models.py` — `KnowledgeDocument` (có `authority_class` — phân biệt nguồn chính thống vs tham khảo) và `KnowledgeChunk` (`content_hash`, `chunker_name/version`, `embedding_model/version` — cho phép biết chunk này được tạo bằng thuật toán/model nào, quan trọng khi đổi chunker/embedding model sau này).
2. Nếu nguồn có API/DB riêng → viết provider mới implement `KnowledgeStore` Protocol (`packages/agent_core/knowledge/store.py`) tương tự `PostgresKnowledgeStore` (`packages/agent_core/knowledge/providers/postgres.py`).
