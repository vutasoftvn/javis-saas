...
`packages/agent_integrations/<runtime-name>/kernel.py` — KHÔNG đặt trong `packages/agent_core/kernel/` (chỉ `OpenAIAgentsKernel` hiện tại ở đó do lịch sử; runtime mới đi vào `agent_integrations` theo `ADR-RUNTIME-ADAPTERS.md`).

## Các bước

1. Đọc `packages/agent_core/contracts/kernel.py` — implement đúng Protocol `ExecutionKernel` (`run`, `resume`, `cancel`, `stream`).
...
4. Model call lỗi → raise `AgentRuntimeError` (`packages/agent_core/contracts/errors.py`), KHÔNG convert thành assistant text thành công (anti-pattern đã fix ở `_call_model()`).
5. Dùng `PromptBundle` (`packages/agent_core/prompts/bundle.py`) để render system message — không tự ghép `spec.instructions` trần.
...
7. Đăng ký trong `apps/cosa/composition/agent_plane.py` qua nhánh `runtime=` param — lazy import, không import runtime mới ở module scope của `agent_core`.
