...
| Workforce Agent Gateway stack | backend/app/workforce/gateway (AgentGateway, RiskPolicyEvaluator, gateway/approval.py::ApprovalService), auto_register.py's register_all_domain_tools and register_extension_tools() | Audit required | grep confirms AgentGateway(...) is constructed only in 2 test files (test_agent_platform_conformance.py, test_google_search_agent.py) -- auto_register.py references the name only as a type hint, never instantiates it; register_all_domain_tools(...) is called only from those same 2 test files, never from production code or from auto_register.py itself; register_extension_tools() has zero callers anywhere in the repo (not even in tests) and its body is a no-op (pass); gateway/approval.py::ApprovalService collides in name with the real agents/governance/approval_service.py::ApprovalService | No new production code should depend on AgentGateway | Confirm zero production caller via evidence command below, then either wire into Phase B's unified ToolInvocationService or formally retire |
