...
`agentos/` was introduced by ADR-AGENTOS-001 as an explicitly **inert, parallel** runtime — not wired into any production entrypoint. `services/` (Encore) has Phase 1 schema/logic parity for `identity`/`operations`/`commercial`/`finance-legal`, but per the `services/operations` cluster plan, **no consumer (backend, frontend, or realtime_agent) calls it over HTTP yet**. Treat both as pre-production until an end-to-end integration test exists that exercises agent → `services/` → caller. `legacy/backend` (`brain-api`/`agent-worker` via the root `docker-compose.yml`) is **frozen-in-place** (user-confirmed decision, 2026-08-22, see ADR-012 "Correction #2"): the restructure commit `5c5bc85` split it across 6 sibling `legacy/*` directories (`backend`, `agent_runtime`, `platform`, `business`, `domains`, `entrypoints`) without updating the Docker build, so it is known-broken today and is **not** being resurrected. `migrate`/`migrate-control-plane`/`brain-api`/`agent-worker` are gated behind `docker compose --profile legacy` so they no longer start by default. Effort instead goes into extracting the specific pieces `agentos/` needs (LLM Gateway, OAuth, n8n, Sandbox) as adapters — see ADR-012 Decision.
