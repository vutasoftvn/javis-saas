...
**Update (2026-08-22, Giai đoạn 2 pilot, see `docs/architecture/AI_AGENT_OS_GAP_ANALYSIS.md`):** `services/` now has its first verified real consumer. `agentos/tools/registry.py` (`register_cluster_tools`) + `agentos/tools/encore_client.py`, invoked through the `task_create`/`task_list`/`task_update_status` tool bindings, were exercised against a live `encore run` instance of `services/` over real HTTP (not a mocked `EncoreClient`) — workspace creation, task creation with idempotency-key retry, and status transition to `done` all round-tripped through Postgres correctly. Integration test: `tests/agentos/test_services_pilot_e2e.py` (skips automatically if no live Encore server is reachable at `:4000` — does not block CI without the dev stack running). While wiring this pilot, a real gap was found and fixed: `shared/events.ts` defined `TASK_CREATED = "task.created"` but nothing ever published it — `createTask` (`services/operations/task.ts`) now publishes it on a genuine insert only (guarded via Postgres `xmax = 0`, so an idempotency-key retry does not re-publish), verified via `vi.spyOn` in `services/operations/task.test.ts` (full `services/` suite 117/117 pass, `tsc --noEmit` clean). This is **still not full production parity** — `services/` is still not called from `legacy/agent_runtime`, `frontend/`, or `realtime_agent`. **Update (2026-08-22, same day):** the pilot was extended to `services/commercial` (`lead_create`/`lead_list`/`lead_update_stage`) and the rest of `services/operations` (`okr_cycle_create`/`okr_objective_create`/`okr_key_result_update_progress`/`twelve_wy_plan_create`/`initiative_create`) — 9/9 pass, `tests/agentos/test_services_pilot_e2e.py`. **All 8 `operations` tool bindings are now pilot-verified over live HTTP.** `commercial.sales_leads` has no `idempotency_key` and no domain event yet — deliberately not added, since inventing lead-lifecycle event semantics is a business decision, not a mechanical gap fix. Treat the rest of `services/` (commercial beyond leads, finance-legal beyond the ADR-012 idempotency work, identity beyond workspace-create) as still needing the same kind of live-HTTP verification before relying on "Phase 1 parity" claims for those clusters.
