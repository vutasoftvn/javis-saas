...
|---|---|---|---|
| Model provider | backend/app/workforce/adapters and workforce agents reliability model gateway | Provider interface, model policy, telemetry | backend/agent_runtime/models provider scaffold |
| Runtime adapter | backend/app/workforce/agents/runtime/adapters | Runtime manager, session/run scope, cancellation | Business Core import of vendor/DSH runtime |
| COSA business tool | backend/app/core/tool_registry.py with backend/app/workforce/tools backend | Tool schema, ExecutionScope, GovernanceKernel, audit | backend/tools registry |
| MCP or external connector | backend/app/workforce/tools/transports through Extension Registry | Tool Invocation Pipeline, secret broker, policy | direct PluginHost execution or direct HTTP call from UI |
| Skill | backend/app/workforce skill lifecycle and protected-resource path | Version, human approval, profile eligibility | backend/skills repository |
| Executor | backend/app/workforce/agents/execution manager/provider | Sandbox, secret broker, policy, artifact/event result | backend/executors stub |
| Workflow graph/compiler | backend/app/integrations/workflows | Graph validation, immutable version, scope, policy | backend/workflows engine as new product workflow authority |
| Workflow node/UI | backend/app/integrations/workflows plus frontend/lib/modules/workflows | Node schema, compiler binding, backend authorization | second workflow canvas/module |
