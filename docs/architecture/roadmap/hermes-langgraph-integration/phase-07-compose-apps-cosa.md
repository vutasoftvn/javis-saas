...
5. Reusability gate check: viết 1 script/test riêng compose "app thứ hai" giả lập, chỉ dùng `RunService`/`ExecutionKernel`/`WorkflowEngine`/Capability contract/Events/Governance/Memory-Knowledge ports từ `packages/agent/`, KHÔNG import gì từ `services/company`. Nếu import bị cần thì boundary chưa đạt — sửa lại `packages/agent/` trước khi tuyên bố Phase 7 xong.

## Bổ sung Hermes/LangGraph — Context Assembler thật

**Nguyên tắc:** salvage invariant từ audit Phase 0 (`docs/architecture/CONTEXT_ASSEMBLER_AUDIT.md`), KHÔNG port code legacy nguyên xi (vi phạm boundary `agent` không import business domain — legacy `CofounderContextAssembler` query trực tiếp SQLAlchemy business models).
...
- `grep -r "services.company" packages/agent/` → rỗng.
