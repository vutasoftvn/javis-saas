...
| M3 | [M3-workspace-vault.md](./M3-workspace-vault.md) | §9.3 | Bắt đầu — ✅ `WorkspaceObjectStore` + `LocalFilesystemWorkspaceStore` (§2/§3/§6.9: path-security, cross-workspace isolation, checksum, no cross-ws dedup; negative suite 26). ⏳ S3 store, RLS + knowledge provider workspace-scope (§4), DEK/key-rotation (§6), `brain_id` removal (§7), backup/export (§9) |
