...
| M4 | [M4-lifecycle.md](./M4-lifecycle.md) | §9.4 | Đang làm — ✅ §1 rename `company_stage`→`lifecycle_stage` (W0–W5) + tách `stage_transition_policies`/`workspace_stage_transitions`; ✅ §2 CAS + versioned policy + provenance + same-stage no-op; ✅ §3 Project P0–P6 lifecycle độc lập (CAS/no-op/journal + assessor P-set + independence tests); ✅ §5 legal entity status v2 (DRAFT..DISSOLVED, drop `platform_company_id`, bỏ workspace `legalStatus` aggregate; 517/517 encore test). ⏳ §3 C-6 online ID mint, §4 frontend route/enum, §6 eligibility |
