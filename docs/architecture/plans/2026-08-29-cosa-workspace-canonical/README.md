...
| M5 | [M5-remote-access.md](./M5-remote-access.md) | §9.5 | Bắt đầu — ✅ §1 runtime node registration + device key + heartbeat + computed presence (`cosa/22`, 138/138 encore test); ✅ §4 authenticated command envelope (HMAC + clock-skew + nonce replay cache, `agent_core/remote`, 11 tests); ✅ §3 Runtime Router decision core + `POST /cosa/runtime/route` wiring (REMOTE_ACCESS không cloud-failover); ✅ §1/§3/§4 HTTP surface + relay command gate audit (`/cosa/runtime/nodes/*`, `RelayCommandGate`; 156/156 encore, 569 agent_core). ⏳ §2 tunnel transport, §5 offline UI, §6 frontend |
