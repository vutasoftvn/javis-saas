...
| M4 | [M4-lifecycle.md](./M4-lifecycle.md) | §9.4 | Đang làm — ✅ §1 rename `company_stage`→`lifecycle_stage` (W0–W5) + tách `stage_transition_policies`/`workspace_stage_transitions`; ✅ §2 CAS + versioned policy + provenance + same-stage no-op; ✅ §3 Project P0–P6 lifecycle độc lập (schema + `transitionProjectStage` CAS/no-op/journal + assessor P-set + independence tests, 517/517 encore test). ⏳ §3 C-6 online ID mint, §4 frontend route/enum, §5 legal status, §6 eligibility |
