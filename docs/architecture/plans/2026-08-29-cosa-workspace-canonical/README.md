...
| M3 | [M3-workspace-vault.md](./M3-workspace-vault.md) | §9.3 | Đang làm — ✅ `WorkspaceObjectStore` + `LocalFilesystemWorkspaceStore` (§2/§3/§6.9: path-security, cross-workspace isolation, checksum, no dedup; 26 neg tests); ✅ knowledge `get_document` bind workspace (§4 phần); ✅ per-workspace DEK + rotation + destroy (§6 key: `WorkspaceKeyManager`, AES-256-GCM, workspace_id AAD, rotation journal; 9 tests); ✅ `brain_id` removal khỏi `frontend/lib/` (§7). ⏳ S3 store (§2), RLS policy + pool reset (§4), SOP lifecycle (§5), storage quota + key wiring (§6 phần còn), workspace switcher (§8), backup/export (§9) |
