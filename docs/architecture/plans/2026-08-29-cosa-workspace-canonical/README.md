...
| M4 | [M4-lifecycle.md](./M4-lifecycle.md) | §9.4 | Đang làm — ✅ §1 rename `company_stage`→`lifecycle_stage` (W0–W5) + tách `stage_transition_policies`/`workspace_stage_transitions`; ✅ §2 CAS trên `stage_version` + versioned policy + provenance/evidence snapshot + same-stage no-op (511/511 encore test). ⏳ §3 project P0–P6, §4 frontend route/enum, §5 legal status, §6 eligibility |
