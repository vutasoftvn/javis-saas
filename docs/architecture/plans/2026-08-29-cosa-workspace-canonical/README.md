...
| M3 | [M3-workspace-vault.md](./M3-workspace-vault.md) | §9.3 | Bắt đầu — ✅ `WorkspaceObjectStore` + `LocalFilesystemWorkspaceStore` (§2/§3/§6.9: path-security, cross-workspace isolation, checksum, no dedup; 26 neg tests); ✅ knowledge `get_document` bind workspace (§4 phần). ⏳ S3 store, RLS policy + pool reset (§4), DEK/key-rotation (§6), `brain_id` removal (§7), backup/export (§9) |
