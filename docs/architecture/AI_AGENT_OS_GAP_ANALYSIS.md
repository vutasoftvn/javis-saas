...
- `skillpacks/{tasks,okr,twelve-week-year,marketing/*,core/weekly-review}/` — skill thật (`manifest.yaml` + `SKILL.md`). **[Audit 0.4 — xác nhận]** `SkillRegistry.discover()` (`agentos/skills/registry.py:35-47`) đọc trực tiếp filesystem và đánh dấu ACTIVE ngay, **không gọi** `supply_chain/pipeline.py`. Pipeline scan/eval/approval (`agentos/skills/supply_chain/pipeline.py:21-70`) tồn tại và chạy được nhưng chỉ dùng cho EXTERNAL skill candidate, chưa từng áp dụng cho skill nội bộ. **Đây là lỗ hổng an toàn cụ thể**, không chỉ thiếu tính năng: mọi skill trong `skillpacks/` hiện bỏ qua toàn bộ static/security scan mà blueprint Phụ lục A §13–§14 yêu cầu bắt buộc.
...
| Skill Ecosystem | ✅ đầy đủ nhất, **nhưng supply_chain chưa wire vào registry (lỗ hổng an toàn xác nhận, chưa sửa)** | ❌ không tồn tại | N/A | Thấp trùng lặp, **cao rủi ro an toàn** |
...
| Event/Workflow | ✅ retry/compensation/parallel đã có trong `agentos/workflows/` (Giai đoạn 3.1–3.3) | `legacy/backend/integrations/workflows` vẫn có version history mà agentos chưa port (ADR-015, chưa làm) | ❌ | Cao |
| Governance/Permission | ✅ khung + **audit log bền vững (Giai đoạn 3.4)**; RBAC vẫn thiếu; PermissionLevel migration (ADR-014) chưa làm | ✅ vocabulary khác | ❌ | Cao |
