...
**Gap lớn nhất của toàn bộ phân tích (đã đóng cho mọi tool binding hiện có):** `services/` đã sẵn sàng schema+logic nhưng ban đầu **0 consumer** — thiếu wiring, không thiếu code. Giai đoạn 2 (+ 3 lần mở rộng cùng ngày) đã verify pilot HTTP thật cho **mọi tool binding còn hoạt động của cả 4 cluster** (`operations` 8/8, `commercial` lead+account+contact+opportunity, `finance-legal` transaction+accounting-period+obligation+checklist, `identity` workspace_get) qua `agentos/tools/encore_client.py` (`tests/agentos/test_services_pilot_e2e.py`, 15/15 pass). Khi mở rộng, phát hiện + sửa thêm 4 tool binding hỏng (`workspace_list`, `organization_get`, `workforce_member_list`, `opportunity_list` — gọi route không tồn tại, cùng loại bug ADR-012 từng tìm cho OKR/12WY) — đã gỡ, không redirect. `customer`/`billing`/`marketing` chưa có tool binding nào (không phải gap pilot).
