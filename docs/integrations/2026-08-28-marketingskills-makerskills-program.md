...
| Mâu thuẫn contract skillpack giữa tài liệu | Đúng. `docs/development/add-skill.md:17` nói `skill.yaml` tại `packages/agent/skills/library/<id>/`. Validator + hardening design + skills.md + cả 16 pack thực tế dùng `manifest.yaml`+`SKILL.md` tại `skillpacks/<domain>/<id>/`. | `packages/agent/skills/skillpack_contract.py`, `docs/features/skills.md:30` |
...
| C3 | `operations.loop-hardening` | loopify | Runbook + test template: idempotency key, transaction, retry/rate limit, first-run verification, bail-out. Trỏ `packages/agent/coordination/scheduler.py` + `runs/leases.py`; cấm cron local / self-wakeup trong prompt. |
