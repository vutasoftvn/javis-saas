...
**Follow-up implemented (2026-08-21, same day, explicit founder direction: "dùng toàn bộ snowflakeid nhé" — hard cut, not a transition window):** the `Union[UUID, int, str]` compromise above was superseded before it shipped. Every identifier field sourced from the Central Control Plane (`company_id`, `platform_project_id`, `event_id`, `PlatformUserSync.id`, `CompanySync.id`, `MembershipSync.user_id`, `FormSubmissionPayload.submission_id`) in `backend/app/platform/sync/schemas.py` is now plain `str` — no UUID type anywhere. `entitlement_crypto.py`'s `uuid.UUID(...)` casts are removed (production code path, used by the real signing/verification flow via `entitlement_manager.py`/`router.py`). `outbox_service.py`'s `uuid.uuid4()` generation is replaced with `generate_snowflake_id()` (this module turned out to have zero production consumers — only its own test file — so this was a correctness fix for dead-but-tested code, not a live behavior change). All 4 affected test files updated to match (`test_platform_sync_schemas.py`, `test_entitlement_enforcement.py`, `test_platform_outbox_service.py`, `test_ed25519_entitlement.py`).

Also fixed opportunistically: `make boundary-check`'s uuid-ban regex was double-escaped (`uuid\\.`) and matched nothing — silently passing while this exact violation sat in production code. Corrected to `uuid\.` with an explicit `--glob` exclude list for 21 other pre-existing files that also use `uuid` (mostly correlation IDs / external protocol requirements like MCP — not audited in this pass, tracked as separate follow-up debt so fixing this check didn't silently break CI over unrelated code).

Also found and fixed while verifying via `docker compose up --build` ("chạy lại database luôn"): a real circular import (`app.core.toolset_resolver` ↔ `app.workforce.agents.runtime.adapters.deepseek_harness`) was crash-looping the `agent-worker` container on every startup — pre-existing, unrelated to the ID fix, found only because this was the first time this session actually ran the full container stack. Fixed by deferring the import inside the one method that uses it. Verified: `docker compose up --build -d` → `/ready` reports all 4 checks `ok`, `agent-worker` container stable (no longer restart-looping) and observed actively dispatching real tasks with Snowflake IDs in its logs.

Full backend suite re-verified after all of the above: 1546 passed, 42 skipped (same as before these changes — no regression). Two tests (`test_delegation_api_is_workspace_scoped_and_retry_is_append_only`, and non-deterministically one other) started failing intermittently only *after* the `docker compose` restart — root-caused to these tests asserting absolute row counts (e.g. `dead_letters == 1`) against the real local dev Postgres (`DATABASE_URL=postgresql://javis:javis@localhost:5432/javis`, not an isolated test DB), which the now-unstuck `agent-worker` is concurrently writing real rows into. Confirmed via 3 separate full-suite runs (different test failed each time) — this is pre-existing test-isolation debt unmasked by fixing the crash loop, not a regression from the ID/import fixes. Not fixed here (would mean redesigning these tests' DB fixture to use an isolated transaction/session, out of scope for this task) — flagged as a new follow-up item.
...
- [x] **Step 4:** File the actual merge/redirect as a fresh task once the decision is made — do not implement it speculatively here. (Decision was "keep both, don't merge" — no merge/redirect task needed. Instead, the boundary itself was made durable: added a docstring to `AgentTaskDispatcher` and `dispatch_agent_task()` each pointing at the other and stating the boundary, so it survives in code, not only in the Ownership Map.)

**Third path found and removed while auditing this (2026-08-21):** `app.founder_os.tasks.task_dispatcher.dispatch_pending_tasks()` — called every cycle from `worker_main.py`'s background loop — turned out to be a confirmed no-op: it queried `Task(status="todo")` rows and logged them, but the actual status-flip/execution logic was commented out from the start ("Actual execution logic would be deferred to workflow_runtime") and never implemented. Consumer report: exactly one call site (`worker_main.py`), no other imports anywhere (grep-verified). This wasn't one of the two real dispatch paths analyzed above — it did nothing at all. Removed the file and its call site; added a row to the Ownership Map marking it removed with the reasoning, so it doesn't get "rediscovered" as ambiguous later.
