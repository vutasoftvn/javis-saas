...
- **Task 5 `5582a6e1` — DONE.** Migrations `services/cosa` 16/17: `scheduled_tasks` + child edges (`parent_task_id`/`depends_on`/`join_policy`/`join_quorum`/`completion_key` + `blocked` status). `child-scheduler.service.ts` (`scheduleChildTask`/`listChildren`/`completeChild` idempotent/`resolveJoin` all·any·quorum) + 4 `expose:false` endpoints. `HttpControlPlaneSchedulerClient` gains `schedule_child_task`/`list_children`/`complete_child` (khớp `ChildSchedulerProtocol`). 6 TS + 3 Python contract tests.
- **Task 6a `3d66af07` — DONE.** `GET /events/metrics?workspaceId=` — live gauges từ `integration.event_outbox` (backlog + oldest-pending-age, claimed, retrying, dead-letter, delivered-24h, active event types). Workspace-scoped. Capacity-review doc trỏ vào nó.
- **Task 6b `2a399ec5` — DONE.** `EmbeddingProvider` protocol + `HashingEmbeddingProvider` (deterministic dev) + `SentenceTransformerEmbeddingProvider` (lazy, offline). `PostgresKnowledgeStore.search_chunks_semantic` chạy pgvector cosine thật (bỏ `NotImplementedError`). `retrieve()` tự tính query embedding từ `embedder` khi eval score đạt ngưỡng. Verified vs pgvector container.

**Toàn bộ 6 task closeout DONE.** Regression: 758 passed / 11 skipped / 0 failed (Python, DB env) + company events 29 + cosa scheduler 13. 2 test infra (`test_crash_recovery_subprocess`, `test_sse_reconnect_e2e`) cần role Postgres `javis_app`/`cosa_control_plane_app` — pre-existing, không liên quan.
