# AI Compliance Production Hardening — Reconciled Plan (post-audit 2026-08-30)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (- [ ]) syntax for tracking.
>
> **Nguồn sự thật:** File này supersede `2026-08-30-ai-compliance-production-hardening.md` cho mục đích thực thi — cùng 10 task, cùng Global Constraints/Release Criteria/Explicitly Deferred, nhưng mỗi task có thêm mục "Ghi chú kiểm chứng" ghi lại bằng chứng thật đã đọc trực tiếp trong code (không chỉ giả định), và một vài điều chỉnh phạm vi dựa trên bằng chứng đó. Đọc "Ghi chú kiểm chứng" trước khi implement — nó chỉ ra chính xác dòng code hiện tại cần sửa.
...
6. **[Bổ sung sau audit]** Nguồn tài liệu pháp lý thật cho Task 6: https://drive.google.com/drive/folders/1de3qEQiTV-ltyL_cA5w3c2JftkZo6uuu (người dùng cung cấp). Hash SHA-256 phải tính thật trên file tải về, reviewer phải là người thật xác nhận — không dùng placeholder.
7. **[Bổ sung sau audit]** Audit event `tool.completed` hiện có ở `packages/agent/capabilities/gateway.py` ghi field `output` không redact — phải sửa trong Task 9 cùng lúc với việc thêm decision event mới, không để leak cũ tồn tại song song.

---

### Task 1: Freeze contract, scope ownership and rejection semantics

**Ghi chú kiểm chứng:** IDOR có thật. `ai-compliance-governance.service.ts`: `submitAiAssessment` (dòng 147-150), `approveAiAssessment` (dòng 190-193, 204-207), `suspendAiDeployment` (325-328), `resumeAiDeployment` (356-359), `getDeployment` (407-410) — tất cả chỉ `WHERE id = ...`, không kèm `workspaceId`. `ai-compliance-snapshot.service.ts`: `verifySnapshotIntegrity` (208-214), `captureComplianceSnapshot` khi có `deploymentIdInput` (51-55) không kiểm tra deployment thuộc `workspaceId` truyền vào. `ai-incident-response.service.ts`: `resolveAiIncident` (146-159) không lọc workspace; `openAiIncident` (129-133) tự suy ra workspace từ deployment id, không kiểm tra quyền. `ai-data-governance.service.ts` phần lớn đúng (đã `and(eq(workspaceId,...))`) — ngoại lệ `withdrawProcessingAuthorization` (223-235) cũng thiếu workspace filter. Test hiện có `returns no deployment from another workspace` (ai-compliance-governance.test.ts:212-216) chỉ test list view (`getComplianceCenterView`), **không** test mutation trực tiếp theo id — nên không phát hiện được các lỗ hổng trên.
...
- Modify: services/company/finance-legal/services/ai-incident-response.service.ts (bổ sung — audit xác nhận file này cũng có lỗ hổng, cần đưa vào cùng đợt sửa)
- Modify: handler tương ứng của 4 service trên (ai-compliance-governance.handler.ts, ai-compliance-snapshot.handler.ts, ai-data-governance.handler.ts, ai-incident-response.handler.ts)
...
Record the five original decisions + the two audit-driven additions above, and the invariant below.
...
- [ ] **Step 2: Add hostile workspace tests for all 4 services, then run them red**

Không chỉ deployment như ví dụ gốc — thêm case cho `verifySnapshotIntegrity`, `resolveAiIncident`, `withdrawProcessingAuthorization`:
...
Put the shared scoped lookup functions in ai-compliance-access.service.ts so governance, data-governance, snapshot and incident-response services cannot drift into separate id-only implementations.
...
**Ghi chú kiểm chứng:** `services/company/shared/db/schema/legal.ts` xác nhận không có composite unique/FK theo `(workspace_id, id)` ở đâu cả — mọi FK chỉ tham chiếu PK đơn `id` (vd dòng 169-171, 189-191, 310-315). `workspaceId` chỉ là cột `bigint().notNull()` độc lập, có index thường để tăng tốc truy vấn nhưng không phải constraint toàn vẹn. Về mặt DB, không gì ngăn một row con trỏ `deployment_id` sang deployment của workspace khác — khớp đúng lỗ hổng IDOR ở Task 1.
...
**Ghi chú kiểm chứng:** `apps/cosa/auth/jwt.py` xác nhận hiện tại không có delegation có cấu trúc — `mint_delegation_token`/`mint_local_delegation_token` (dòng 90-118) chỉ re-sign lại cùng shape `{sub, aud, exp}` (hoặc `{sub, exp}`), không có `workspace_id/run_id/capability_ids/iss`. Đây chỉ là cơ chế giảm rủi ro lộ bearer token dài hạn khi lưu vào durable queue, không phải scoped delegation. `services/company/shared/auth/` chỉ có duy nhất `workspace-access.ts` (`requireWorkspaceAccess`) — hoàn toàn không có khái niệm delegation. Khớp đúng giả định gốc của plan ("current delegation contains only sub, an optional cosa audience, and expiry").
...
**Ghi chú kiểm chứng:** `captureComplianceSnapshot` (ai-compliance-snapshot.service.ts:63-144) tệ hơn mô tả gốc — không chỉ tự tạo `aiSystemCatalog`/`aiSystemVersions`/`workspaceAiDeployments` mặc định (dòng 70-97), mà còn tự set `aiRiskAssessments.status: "APPROVED"` ngay lập tức (dòng 99-111, đặc biệt dòng 108) khi tự sinh assessment, bỏ qua toàn bộ precondition của flow `approveAiAssessment` thật, và gán `founderMemberId: generateSnowflake()` — một ID giả không map tới member nào.
...
Expected: FAIL because captureComplianceSnapshot creates default system, deployment, and an APPROVED assessment.
...
**Ghi chú kiểm chứng:** `RunRequest` hiện tại (apps/cosa/worker/handlers.py:182-190) chỉ mang `metadata={"policy_snapshot": ...}` — không có compliance/delegation metadata riêng. `ComplianceResolver.resolve_for_run` được gọi bên trong kernel (packages/agent_integrations/openai_agents_sdk/kernel.py:303-305), tức SAU KHI worker đã gọi `plane.kernel.run` — không phải ở bước schedule như thiết kế mong muốn. `InvocationContext.delegation_identity` (packages/agent/contracts/invocation.py:45) đã tồn tại làm field nhưng chưa có nơi nào gán giá trị. `agent_plane.py` có `_MockAiComplianceClient` (dòng 556-572) tự kích hoạt bằng cách kiểm tra `isinstance(client, AsyncMock/MagicMock)` ngay trong composition code thật — cần thay bằng dependency injection tường minh khi wire delegation thật.
...
Company capability handlers build these values from InvocationContext, never tool arguments. OpenAI kernel propagates delegation_identity, compliance_snapshot_ref and policy snapshot reference into every InvocationContext. Replace `_MockAiComplianceClient` type-sniffing with explicit dependency injection (env flag or factory), not runtime isinstance checks in production composition code.
...
**Ghi chú kiểm chứng:** Migration 28 xác nhận `content_hash` của cả 8 nguồn luật đã seed đều giống hệt nhau và bằng SHA-256 của chuỗi rỗng (`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`) — placeholder giả hoàn toàn, không phải bằng chứng thật. Rule engine (ai-legal-applicability.service.ts:34-49) hard-code từ khóa tiếng Anh+Việt trong TS, không liên kết với `applicabilityRules` đã seed ở DB.

**⚠️ Nguồn tài liệu thật:** https://drive.google.com/drive/folders/1de3qEQiTV-ltyL_cA5w3c2JftkZo6uuu (người dùng cung cấp). Bước đầu tiên là tải văn bản gốc thật từ thư mục này, lưu artifact cục bộ, tự tính SHA-256 thật trên file đã tải. **Không dùng placeholder, không tự suy đoán nội dung/citation.** Vẫn cần người dùng xác nhận vai trò reviewer pháp lý (reviewerMemberId, ngày review) trước khi set reviewStatus là đã duyệt. Nếu thư mục thiếu văn bản nào hoặc nội dung không rõ ràng, dừng lại hỏi thay vì suy đoán.
...
Insert corrected metadata only after named legal reviewer provides exact source artifact and hash (đã có từ Step 1). Set superseded/incorrect version records inactive with a correction reason; retain audit history. Add normalized source-version relations to evidence/rules and fields needed for conclusion/pinpoint.
...
**Ghi chú kiểm chứng:** `apps/cosa/compliance/data_access_claim.py` không tồn tại (grep toàn repo không có kết quả). `CosaDataModelGate.prepare_initial_input` (data_model_gate.py:19-44) hard-code `data_categories=["PERSONAL", "BUSINESS_CONFIDENTIAL"]` (dòng 36) cho mọi request, `purpose_id`/`provider_key` dùng default cứng (`"advisory"`, `"deepseek"`). `assert_before_model_call` (dòng 58-64) chỉ check `status != "APPROVED_FOR_USE"`.
...
**Ghi chú kiểm chứng:** 4/5 endpoint Flutter sai path prefix hoàn toàn: gọi `POST /finance-legal/ai-deployments/:id/{suspend,resume,approve}` và `POST /finance-legal/ai-incidents`, trong khi backend thật là `POST /finance-legal/ai-compliance/deployments/:id/{suspend,resume,approve}` và `POST /finance-legal/ai-compliance/incidents`. `approve` thiếu field bắt buộc `assessmentId`+`expiresAt`; report-incident thiếu field bắt buộc `incidentType`. Model Dart `AiComplianceCenterData`/`AiComplianceDeployment` đọc field hoàn toàn không tồn tại trong response server thật (`recentIncidents`, `activeCount`, `incidentCount`, `ownerName`, `assessmentExpiresAt`, `providerStatus`, `allowedCapabilities`) — response thật trả `deployments/assessments/providerProfiles/incidents` với field khác (`founderMemberId`, `technicalOwnerMemberId`, `currentAssessmentId`...). Test widget hiện tại mock toàn bộ service nên không phát hiện được mismatch. `WorkspaceScopedService` gắn `workspace_id` làm query param đồng thời `ApiClient` gắn header `X-Workspace-Id` — gửi kép, cần rà lại.
...
Use existing WorkspaceService header behavior and assert X-Workspace-Id in test. Do not append workspace_id query text as a replacement header — resolve the current double-send (query param + header) to a single mechanism. Approve only when a current pending assessment and expiry are available. Render server rejection reason instead of optimistic success.
...
**Ghi chú kiểm chứng:** `CapabilityGateway._execute_internal` (packages/agent/capabilities/gateway.py) đã tạo structured audit event qua `RunEventRecord` (`tool.requested`, `policy.evaluated`, `tool.started`, `tool.completed`, `tool.failed`) — không thấy field nào ghi token/bearer/prompt trực tiếp. **Nhưng sự kiện `tool.completed` (dòng 574-580) ghi nguyên văn field `output` không redact** — nếu capability handler trả dữ liệu PII/business-confidential thô, nó bị ghi thẳng vào audit event log persist Postgres, độc lập với decision-event mới mà task này định thêm. Đây là phát hiện bổ sung ngoài phạm vi mô tả gốc — phải sửa cùng lúc, không chỉ thêm field mới bên cạnh field leak cũ.

**Files:**
- Modify: services/company/finance-legal/services/ai-compliance-snapshot.service.ts
- Modify: packages/agent/runs/models.py
- Modify: packages/agent/capabilities/gateway.py (bao gồm sửa `tool.completed` tại dòng ~574-580 để redact/hash `output` trước khi ghi)
...
- It never contains Authorization, company_delegation, prompt, completion, raw subject reference, document content or unredacted model payload — this also applies to the pre-existing tool.completed event.
...
Bổ sung case riêng khẳng định `tool.completed.output` cũng không chứa PII thô sau khi sửa.

- [ ] **Step 2: Add dedicated compliance decision payload**

Extend existing run/tool event payloads and CapabilityGateway structured event creation; do not create a second event store. Fix `tool.completed` to redact/hash `output` before persisting (reuse existing Redactor/hash utility, do not build a second redaction mechanism). Logging uses allowlist serialization so unknown metadata keys, including delegation tokens, are excluded by default.
...
**Ghi chú kiểm chứng:** `.github/workflows/quality.yml` không có job nào đề cập `ai-compliance`, `finance-legal`. `Makefile` có `ai-compliance-test`/`ai-compliance-smoke` nhưng chỉ chạy Python (`tests/apps/cosa/compliance`) + 1 Flutter widget test dùng mock — không chạy TS finance-legal test, không test HTTP thật. Không có target `ai-compliance-production-gate`. Không có ADR nào tên `ADR-AI-COMPLIANCE-RUNTIME-001` trước Task 1.
...
- **Audit trail của bản reconciled này:** mọi "Ghi chú kiểm chứng" ở trên được xác nhận bằng cách đọc trực tiếp code hiện tại (không phải chỉ đọc plan gốc) — xem hội thoại triển khai 2026-08-30 để biết chi tiết agent đã đọc file/dòng nào.
