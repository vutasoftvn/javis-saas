...
- **Phase C — Flutter clean-slate.** ONE atomic change (interdependent — `flutter analyze` only goes green once everything below lands together):
  - Delete `frontend/lib/modules/strategy/` (84 files) + `frontend/test/**/strateg*` (26 files). Only importers outside the module are `lib/core/routing/{module_routes,app_pages}.dart`.
