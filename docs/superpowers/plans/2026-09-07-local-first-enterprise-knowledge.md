...
Gap biết trước LÚC LÀM Task 9, không phải thiếu sót: `WorkspaceContextOperation` KHÔNG gọi "read-only business adapter" nào — plan không đặc tả 1 contract cụ thể (không có capability read tổng hợp business context sẵn có để compose mà không tự bịa schema). `business` field trả về rỗng tường minh (`{}`), chỉ `citations` (từ `enterpriseKnowledgeSearch`) là thật. Mở rộng business context là việc tương lai ngoài scope Task 9.

**Đã đóng 1 phần** (theo yêu cầu người dùng, sau khi rà soát toàn phiên): wire `operations.task.list` — capability read-only ĐÃ có sẵn, workspace-scoped (không cần tham số nào ngoài `workspace_id`, đúng shape `workspaceContext` đang có). Gọi THẲNG handler (không qua `CapabilityGateway` riêng — cùng cách `EnterpriseKnowledgeSearchOperation` đã làm cho citations: 1 lần gọi `workspace.context.read` = 1 entry audit, nguồn đọc bên trong là chi tiết triển khai). `business` giờ có `{"tasks": [...]}` (tối đa 5 task, fail-closed rỗng nếu thiếu `company_client` hoặc company service lỗi — không làm hỏng citations đã có). Test mới: `test_founder_workspace_context_includes_permitted_business_and_knowledge` (giờ assert thật `business.tasks`, trước chỉ assert citations dù tên test nói "business_and_knowledge"), `test_workspace_context_business_is_empty_without_company_client`, `test_workspace_context_business_fails_closed_when_company_service_errors`.

**Vẫn CHƯA wire** (đánh giá rõ trong docstring `WorkspaceContextOperation`): `strategy.next_best_action.get`/`strategy.project.get`/`analytics.pmf_scoreboard.get` và các capability strategy/analytics khác — TẤT CẢ đều yêu cầu `project_id` cụ thể trong `input_schema`, không có biến thể workspace-level. `workspaceContext` chỉ nhận `question`+workspace, không có project nào được chọn sẵn — tự đoán "project đầu tiên/chính" sẽ suy diễn sai khi workspace có nhiều project (không phải cách governance-là-code-xác-định, CLAUDE.md rule 5). Cần 1 trong 2: (a) quyết định sản phẩm cách chọn project mặc định cho workspace, hoặc (b) mở rộng `allowed_variables` của `workspaceContext` nhận `project_id` tường minh từ caller — cả hai đều ngoài phạm vi 1 lần sửa gap, để lại cho quyết định sau.
...
- [x] **Step 5: Run frontend and API contract gates.** `flutter test test/modules/vault/` (9 test PASS) + `flutter test` đầy đủ (1487 passed — sửa 1 test khác hardcode `vaultDocumentList.enabled==false` làm fixture, nay route đã enabled, đổi sang `vaultKnowledgeGraph` còn disabled) + `flutter analyze` sạch; `make frontend-api-contract-check` (12 test Python, sửa 2 test tương tự hardcode literal `/agent/vault/documents` giờ đã enabled) + `make mvp-surface-check` + `make mvp-contracts-check` + `make contracts-check` PASS; backend `tests/apps/cosa/` đầy đủ 951 passed/27 skipped, `make typecheck-py` sạch (352 file).
  ...
  Gap phát hiện lúc kiểm tra kỹ hơn ngoài phạm vi gate CI thật (không phải lỗi do Task 12 gây ra): `mvp_surface_check.py --ledger` tham chiếu `docs/superpowers/plans/2026-08-31-full-mvp-acceptance-ledger.md` — file này không tồn tại từ trước, và rà soát sâu hơn cho thấy **không có gate CI/`make verify*` nào từng gọi `--ledger`** (kể cả `mvp-surface-check` không `--ledger` cũng không nằm trong `make verify`) — toàn bộ `validate_acceptance_ledger()` là code chết trong pipeline thật dù có unit test riêng. Theo yêu cầu người dùng, đã XOÁ hẳn tính năng `--ledger`/`validate_acceptance_ledger()`/test liên quan (dọn dẹp 1 tính năng dự định làm dở từ 2026-08-31, không phải giữ lại 1 file 120 dòng chỉ để thoả mãn 1 code path không ai gọi).
