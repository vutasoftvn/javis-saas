# M4 — Ranh giới Promotion giữa agent và services/cosa
...
`packages/agent/evals/` (Wave M4) sở hữu:
...
`agent` **KHÔNG** sở hữu và **KHÔNG BAO GIỜ** tự triển khai:
...
## Luồng dự kiến (services/cosa gọi vào agent, KHÔNG phải chiều ngược lại)

```text
services/cosa nhận yêu cầu promote (người dùng/API)
   │
   ▼
Query agent: PromotionEvidenceRepository.list_by_target(target_ref)
   │
   ▼
Query agent: PromotionGate(policy_version).check(evidence, current_fingerprints)
   │
   ...
   agent, không nằm trong phạm vi Wave M4.
```
...
- **Compliance/audit**: 1 quyết định "đưa gì lên production" phải truy vết được về đúng 1
  ...
  Python runtime state của `agent` (có thể restart, có nhiều instance chạy song song).
- **Không để LLM/agent runtime tự cấp quyền cho chính nó** — nếu `agent` tự activate
...
- `agent` phía (Task 1-5, Wave M4): **triển khai xong** khi plan
