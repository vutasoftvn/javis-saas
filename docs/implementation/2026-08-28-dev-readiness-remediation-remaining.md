...
| P2 §8 — workflow DAG (cycle/dangling/dup) | Xong | commit `dd6185d6`, `adff857b`; `_validate_dag()` (`packages/agent/workflows/schema.py`) reject cycle, dangling dep, dup id, `steps=[]`, và all-compensation. `engine.py` fail-safe chuyển `FAILED` nếu forward steps chưa hoàn tất. |
