...
- **Eval scope làm thật, đầy đủ**: xây `EvalSuite`/`EvalCase`/`EvalRun`/`EvalResult` + `PromotionEvidence` như evidence artifact thật trong `agent`, nhưng quyền quyết định promote (`PromotionDecision`, activation) thuộc `services/cosa` — `agent` chỉ tạo evidence, không tự quyết.
...
2. Xác nhận `definition_hash()` (`packages/agent/governance/hashing.py`) đã canonical đúng §6.2 tài liệu gốc (sort keys, order-independent, loại trừ runtime context) — chỉ cần thêm prefix `sha256:` nếu output hiện tại chưa có.
...
4. Viết golden fixture test cho fingerprint: `tests/agent/test_m0_fingerprint_fixtures.py` — key-order independence, runtime-context exclusion (run_id/region không đổi hash), dependency thay đổi → fingerprint đổi.

**File liên quan:** `packages/agent/governance/hashing.py` (đọc, không sửa), `packages/agent/governance/contracts.py` (đọc, quyết định trước khi sửa ở M1).
...
1. Sửa `packages/agent/governance/contracts.py`: mở rộng `PinnedSpecIdentity.spec_kind` theo quyết định M0-3.
2. Thêm dependency edge model tối giản trong cùng file hoặc `packages/agent/governance/lineage.py` (mới, chỉ nếu M0 xác nhận chưa có chỗ tương đương): `owner: PinnedSpecIdentity`, `dependency: PinnedSpecIdentity`, `relation: str`.
3. Thêm adapter method trên `PinnedSpecIdentity` để convert từ `PinnedSkillRef` (`packages/agent/contracts/identity.py`) — giữ backward compatibility, không sửa `PinnedSkillRef` nếu không cần.
4. Map lỗi vào `packages/agent/contracts/errors.py` hiện có (mismatch, missing dependency, drift) — chỉ thêm class mới nếu không có tương đương sau khi đọc file này.
5. Viết test: `tests/agent/test_m1_pinned_spec_identity.py` — dependency-drift propagation, version/hash conflict (`SpecVersionHashConflictError` đã có ở `registry/repository.py`), adapter lossless.

**File liên quan:** `packages/agent/governance/contracts.py`, `packages/agent/contracts/identity.py`, `packages/agent/contracts/errors.py`.
...
1. Thêm `PromptSpec`, `ModelPolicySpec`, `ToolContractSpec` theo pattern `AgentSpec.compute_hash()` đã có ở `packages/agent/contracts/spec.py` — file mới `packages/agent/contracts/prompt.py`, `model_policy.py`, `tool_contract.py`. Hiện `model_policy` đang là `dict[str, Any]` thô (`contracts/spec.py:23`).
2. Mở rộng `AgentSpec` (`packages/agent/contracts/spec.py`) thêm `prompt_ref`, `model_policy_ref`, `tool_contract_refs` kiểu `PinnedSpecIdentity`, optional trong giai đoạn transition (không breaking existing callers).
3. Mở rộng `packages/agent/registry/publisher.py`: thêm `publish_prompt_spec()`, `publish_model_policy_spec()`; verify dependency tồn tại trong registry trước khi publish AgentSpec pin chúng.
4. Viết/hoàn thiện resolver "exact resolution" (kind+id+version+expected_fingerprint), tách khỏi "authoring resolution" (floating, chỉ cho UI/draft) theo §7.3 tài liệu gốc — `packages/agent/registry/resolver.py` (kiểm tra trước xem đã có file này chưa, nếu có thì extend).
...
**File liên quan:** `packages/agent/contracts/spec.py`, `packages/agent/registry/publisher.py`, `packages/agent/registry/resolver.py`, `apps/cosa/agents/specs.py`, `apps/cosa/api/routes.py`, `apps/cosa/worker/handlers.py`, `apps/cosa/composition/agent_plane.py` (chỉ đọc để hiểu nơi RunRecord được tạo).

**Test:** `tests/agent/test_m2_registry_integration.py` (publish/resolve prompt+model_policy, AgentSpec fingerprint đổi khi dependency đổi, resolver reject mismatch/missing) + test tích hợp ở `apps/cosa` (Run tạo từ spec resolve qua registry, không phải import trực tiếp constant).
...
1. Xây `EvalCaseSet`, `EvalSuite`, `EvalRun`, `EvalResult` trong `packages/agent/evals/models.py` (hiện chỉ có `EvalTestCase`/`EvalResult` rời rạc, không ref suite/run) — có `to_artifact_ref()` dùng `PinnedSpecIdentity`, fingerprint theo §9.2 tài liệu gốc (case set + scorer version + threshold, loại trừ runtime context).
2. Viết `packages/agent/evals/repositories.py` (hiện không tồn tại) cho persistence các model trên.
...
4. Wire Skill Optimization Lab (`packages/agent/skills/lab/lab.py`, `packages/agent/skills/lab/models.py`): thêm `eval_run_id` vào `SkillCandidateRecord`/`SkillMutationRecord` để lineage skill→candidate→eval evidence traceable thật.

**File liên quan:** `packages/agent/evals/models.py`, `packages/agent/evals/repositories.py` (mới), `packages/agent/evals/runner.py`, `packages/agent/migrations/008_agent_evals.sql` (đọc trước khi viết migration mới nối tiếp), `packages/agent/skills/lab/lab.py`, `packages/agent/skills/lab/models.py`.

**Test:** `tests/agent/test_m3_eval_artifacts.py` — hai `EvalRun` cùng target+suite nhưng khác execution context không tạo hai suite identity khác nhau; suite fingerprint đổi khi case set/scorer version đổi; Skill Lab mutation record trace được sang eval evidence thật.
...
1. Xây `PromotionEvidence` (evidence_id, target ref, required eval_run_ids, observed_fingerprints, policy_version, policy_checks_passed) trong `packages/agent/evals/promotion.py` (mới) — immutable, có `is_stale()`.
2. Xây `PromotionGate` logic (`packages/agent/evals/promotion_gate.py`, mới) — check stale/dependency drift/failed eval, output `PromotionGateResult`, không tự activate gì.
3. Viết migration cho `agent_evals.promotion_evidence` (đọc schema M3 trước để tái dùng cột nếu hợp lý).
4. Viết `docs/implementation/M4_PROMOTION_CONTROL_PLANE_BOUNDARY.md` — contract ranh giới: `agent` export read-only evidence-query API; `services/cosa` sở hữu `PromotionDecision` + activation, đúng CLAUDE.md 4 vùng kiến trúc và §12.3 tài liệu gốc. Không code phía `services/cosa` trong wave này trừ khi user yêu cầu — chỉ document contract.

**File liên quan:** `packages/agent/evals/promotion.py` (mới), `packages/agent/evals/promotion_gate.py` (mới), migration mới nối tiếp M3.

**Test:** `tests/agent/test_m4_promotion_evidence.py` — evidence stale detection khi fingerprint drift sau eval pass; gate reject khi dependency drift/missing/eval fail.

**Exit criteria:** promotion bị block khi evidence stale; không có code nào trong `agent` tự set trạng thái production.
...
1. Audit `packages/agent/workflows/engine.py` (đã có DAG, parallel step, checkpoint/resume, compensation) — xác nhận đủ dùng làm offline eval/build DAG chưa. Ghi kết quả vào `docs/implementation/M5_OFFLINE_DAG_AUDIT.md` trước khi code bất cứ gì.
2. Chỉ nếu audit xác nhận thiếu năng lực: viết `OfflineStep` adapter (`packages/agent/workflows/offline_steps.py`) + artifact-aware cache key (fingerprint + dependency fingerprints, `packages/agent/workflows/artifact_cache.py`).
...
- Không xây `PromotionDecision`/activation trong `agent` — thuộc `services/cosa`.
...
- Mỗi wave có test suite riêng trong `tests/agent/` (unit cho fingerprint/dependency-drift/conflict, integration cho publish→lineage→resolve, và cho Skill Lab→eval lineage, và cho Run thật resolve spec qua registry ở M2).
