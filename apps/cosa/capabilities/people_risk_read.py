...
logger = logging.getLogger("cosa.capabilities.people_risk_read")

__all__ = ["PEOPLE_RISK_READ_SPEC", "create_people_risk_read_handler"]

PEOPLE_RISK_READ_SPEC = CapabilitySpec(
    id="people.risk.read",
    description=(
        "Đọc snapshot People Risk Dossier mới nhất của một Project "
        "(Read-only, chỉ dữ liệu tổng hợp đã phân loại — capacity_bands/"
        "risk_signals/source_refs — không có CV, compensation, protected "
        "characteristic, performance note, health data hay contact PII; "
...
def create_people_risk_read_handler(
    ...
    async def handle(args: dict[str, Any], ctx: Any) -> dict[str, Any]:
        ...
        snapshot = await company_client.get(
            f"/operations/projects/{project_id}/people-risk-dossier",
            headers=headers,
        )
        ...
        # (Task 1) — không thêm bất kỳ PII surface mới nào.
        return {
            "dossierId": snapshot.get("dossierId"),
            "revision": snapshot.get("revision"),
            "capacityBands": snapshot.get("capacityBands", []),
...
def _resolve_project_id(args: dict[str, Any], ctx: Any) -> str:
    ...
    if context_project_id and requested_project_id and requested_project_id != context_project_id:
        raise ValueError(
            "people.risk.read: project_id trong args không khớp scope của run "
    ...
    if not project_id:
        raise ValueError("people.risk.read: thiếu project_id")
    return project_id
...
def _resolve_workspace_id(ctx: Any) -> str:
    ...
    if not wid:
        raise ValueError("people.risk.read: thiếu workspace_id trong ctx")
    return str(wid)
