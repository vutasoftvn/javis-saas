...
logger = logging.getLogger("cosa.capabilities.legal_issue_read")

__all__ = ["LEGAL_ISSUE_READ_SPEC", "create_legal_issue_read_handler"]

LEGAL_ISSUE_READ_SPEC = CapabilitySpec(
    id="legal.issue.read",
    description=(
        "Đọc snapshot Legal Issue Dossier mới nhất của một Project "
        "(Read-only, chỉ dữ liệu tổng hợp đã phân loại — issueCategory/"
        "legalRecordRefs/applicabilityStatus/jurisdiction/redactedQuestion — "
        "không có toàn văn hợp đồng, PII hay tư vấn privileged; không có "
        "quyền tạo/append)."
    ),
    ...
    output_schema={
        ...
        "properties": {
            ...
            "legalRecordRefs": {"type": "array"},
            "applicabilityStatus": {"type": "string"},
            "jurisdiction": {"type": "string"},
            "redactedQuestion": {"type": "string"},
            "status": {"type": "string"},
        },
    },
)
...
def create_legal_issue_read_handler(
    ...
    async def handle(args: dict[str, Any], ctx: Any) -> dict[str, Any]:
        ...
        snapshot = await company_client.get(
            f"/operations/projects/{project_id}/legal-issue-dossier",
            headers=headers,
        )
        ...
        # (Task 1) — không thêm bất kỳ contract-body/PII/privileged-advice
        # surface mới nào.
        return {
            "dossierId": snapshot.get("dossierId"),
            "revision": snapshot.get("revision"),
            "issueCategory": snapshot.get("issueCategory"),
            "legalRecordRefs": snapshot.get("legalRecordRefs", []),
            "applicabilityStatus": snapshot.get("applicabilityStatus"),
            "jurisdiction": snapshot.get("jurisdiction"),
            "redactedQuestion": snapshot.get("redactedQuestion"),
...
def _resolve_project_id(args: dict[str, Any], ctx: Any) -> str:
    ...
    if context_project_id and requested_project_id and requested_project_id != context_project_id:
        raise ValueError(
            "legal.issue.read: project_id trong args không khớp scope của run "
    ...
    if not project_id:
        raise ValueError("legal.issue.read: thiếu project_id")
    return project_id
...
def _resolve_workspace_id(ctx: Any) -> str:
    ...
    if not wid:
        raise ValueError("legal.issue.read: thiếu workspace_id trong ctx")
    return str(wid)
