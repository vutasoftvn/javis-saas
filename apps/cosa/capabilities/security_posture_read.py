...
logger = logging.getLogger("cosa.capabilities.security_posture_read")

__all__ = ["SECURITY_POSTURE_READ_SPEC", "create_security_posture_read_handler"]

SECURITY_POSTURE_READ_SPEC = CapabilitySpec(
    id="security.posture.read",
    description=(
        "Đọc snapshot Security Posture Dossier mới nhất của một Project "
        "(Read-only, chỉ dữ liệu tổng hợp đã phân loại — controlStates/findings/"
        "evidenceRefs/severity — không có secret, credential, raw vulnerability "
        "payload hay infrastructure topology; không có quyền tạo/append)."
    ),
...
def create_security_posture_read_handler(
    ...
    async def handle(args: dict[str, Any], ctx: Any) -> dict[str, Any]:
        ...
        snapshot = await company_client.get(
            f"/operations/projects/{project_id}/security-posture-dossier",
            headers=headers,
        )
        ...
        # (Task 1) — không thêm bất kỳ secret/topology surface mới nào.
        return {
            "dossierId": snapshot.get("dossierId"),
            "revision": snapshot.get("revision"),
            "severity": snapshot.get("severity"),
            "controlStates": snapshot.get("controlStates", []),
...
def _resolve_project_id(args: dict[str, Any], ctx: Any) -> str:
    ...
    if context_project_id and requested_project_id and requested_project_id != context_project_id:
        raise ValueError(
            "security.posture.read: project_id trong args không khớp scope của run "
    ...
    if not project_id:
        raise ValueError("security.posture.read: thiếu project_id")
    return project_id
...
def _resolve_workspace_id(ctx: Any) -> str:
    ...
    if not wid:
        raise ValueError("security.posture.read: thiếu workspace_id trong ctx")
    return str(wid)
