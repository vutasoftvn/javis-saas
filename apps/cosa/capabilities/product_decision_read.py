...
def _resolve_project_id(args: dict[str, Any], ctx: Any) -> str:
    ...
    `project_id` là input bắt buộc theo input_schema (một số caller nội bộ vẫn
    truyền nó tường minh), nhưng model/agent không được phép dùng nó để đọc
    dossier của Project khác ngoài scope run đang chạy. Nếu args["project_id"]
    ...
    if context_project_id and requested_project_id and requested_project_id != context_project_id:
        raise ValueError(
            "product.decision.read: project_id trong args không khớp scope của run "
...
def _resolve_workspace_id(ctx: Any) -> str:
    ...
    if not wid:
        raise ValueError("product.decision.read: thiếu workspace_id trong ctx")
    return str(wid)
