...
class EventTriggerRule:
    ...
    aggregate_filter: dict | None = None
    owner: str = "operator"
    enabled: bool = False
...
class TriggerPolicyService:
    ...
    async def resolve(
        ...
        *,
        ...
        rule: EventTriggerRule | None = await self.store.find(workspace_id, event_type, aggregate)
        ...
        missing = [
            c for c in rule.required_capabilities if not self.capabilities.has(workspace_id, c)
        ]
        if missing:
            return TriggerDecision(
                outcome="policy_denied", reason=f"missing_capability:{missing[0]}"
            )

        # P1/P4 Gate #3: rule proposal/write có eval_evidence_ref thì BẮT BUỘC re-check fingerprint.
