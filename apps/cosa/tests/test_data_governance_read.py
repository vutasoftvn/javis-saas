...
from apps.cosa.capabilities.data_governance_read import create_data_governance_read_handler
...
_CANNED_SNAPSHOT = {
    ...
    "revision": 2,
    "assets": [{"assetId": "asset-1", "classification": "INTERNAL", "qualityStatus": "VALIDATED"}],
    ...
    "rawFileUri": "s3://should-never-leak/data.csv",
...
async def test_handler_uses_ctx_project_and_workspace_scope_with_real_dict_ctx() -> None:
    ...
    handler = create_data_governance_read_handler(fake_client)
    ...
    assert call["path"] == "/operations/projects/project-A/data-governance-dossier"
    ...
    assert result == {
        ...
        "revision": 2,
        "assets": [
            {"assetId": "asset-1", "classification": "INTERNAL", "qualityStatus": "VALIDATED"}
        ],
        "sourceRefs": [{"sourceRef": "source-1", "classification": "INTERNAL"}],
        "status": "CONFIRMED",
    }
    assert "rawFileUri" not in result
