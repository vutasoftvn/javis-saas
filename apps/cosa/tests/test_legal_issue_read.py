...
from apps.cosa.capabilities.legal_issue_read import create_legal_issue_read_handler
...
_CANNED_SNAPSHOT = {
    ...
    "legalRecordRefs": [
        {"recordType": "OBLIGATION", "recordId": "obl-1", "classification": "internal"}
    ],
    "applicabilityStatus": "UNKNOWN",
    "jurisdiction": "VN",
    "redactedQuestion": "Is this vendor clause enforceable?",
    "status": "CONFIRMED",
    # Trường lạ không thuộc output_schema — phải bị lọc bỏ, không passthrough.
    "contractBody": "should never leak",
    "privilegedAdvice": {"secret": "should never leak"},
}
...
async def test_handler_uses_ctx_project_and_workspace_scope_with_real_dict_ctx() -> None:
    ...
    handler = create_legal_issue_read_handler(fake_client)
    ...
    assert call["path"] == "/operations/projects/project-A/legal-issue-dossier"
    ...
    assert "contractBody" not in result
    assert "privilegedAdvice" not in result
