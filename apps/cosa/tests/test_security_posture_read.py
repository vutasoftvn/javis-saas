...
from apps.cosa.capabilities.security_posture_read import create_security_posture_read_handler
...
_CANNED_SNAPSHOT = {
    ...
    "rawVulnerabilityPayload": "should never leak",
    "infraTopology": {"secret": "should never leak"},
}
...
async def test_handler_uses_ctx_project_and_workspace_scope_with_real_dict_ctx() -> None:
    ...
    assert call["path"] == "/operations/projects/project-A/security-posture-dossier"
    ...
    assert "rawVulnerabilityPayload" not in result
    assert "infraTopology" not in result
