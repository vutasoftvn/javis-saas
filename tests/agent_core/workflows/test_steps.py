...
class _EchoAgent:
    async def run(self, task) -> _MockAgentResult:
        return _MockAgentResult(status="COMPLETED", output=f"researched: {task.goal}")


class _FailingAgent:
    async def run(self, task) -> _MockAgentResult:
        return _MockAgentResult(status="FAILED", error="model unavailable")
...
class _LabeledAgent:
    ...
    async def run(self, task) -> _MockAgentResult:
        if self._fail:
            return _MockAgentResult(status="FAILED", error=f"{self._label} failed")
        return _MockAgentResult(status="COMPLETED", output=f"{self._label}: {task.goal}")
