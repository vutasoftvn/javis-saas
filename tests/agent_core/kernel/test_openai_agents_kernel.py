...
from agent_core.contracts.errors import RuntimeErrorCode
...
async def test_kernel_allow_path_tool_execution_preserves_real_run_and_tool_call_id():
    ...
    giả không tồn tại trong agent_core.runs). InMemoryRunRepository không phát hiện
    ...
    kernel = ManualToolLoopKernel(repository=repo, capability_executor=gateway.execute)
