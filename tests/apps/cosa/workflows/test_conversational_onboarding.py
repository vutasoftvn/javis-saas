...
async def test_workflow_session_lifecycle():
    ...
    wf = ConversationalOnboardingWorkflow("ws-life-1", OnboardingSessionType.PARTIAL_UPDATE, client=client)
    ...
    dim_resp = await wf.submit_dimension(
        "stage_scale",
        {"arr": 150000, "burn_rate_weekly": 5000, "runway_weeks": 32, "headcount": 8},
    )
...
def test_event_trigger_detector():
    ...
    r1 = EventTriggerDetector.detect("Chúng tôi vừa nhận được term sheet vòng Seed 500k USD từ quỹ đầu tư mạo hiểm.")
    ...
    r2 = EventTriggerDetector.detect("Tuần tới công ty vừa tuyển một CTO mới để lead đội ngũ engineering.")
    assert r2.triggered is True
    assert r2.event_type == "key_personnel_change"
    assert r2.suggested_dimension == "team_culture"

    # 3. Competitor move
    r3 = EventTriggerDetector.detect("Đối thủ cạnh tranh lớn vừa tung sản phẩm mới và hạ giá 30% để chiếm thị phần.")
    assert r3.triggered is True
    assert r3.event_type == "competitor_move"
    assert r3.suggested_dimension == "market"

    # 4. Strategic pivot
    r4 = EventTriggerDetector.detect("Sau khi test thị trường, ban giám đốc quyết định pivot sang B2B SaaS doanh nghiệp.")
