...
@pytest.mark.asyncio
async def test_upload_creates_private_document_for_member_ignoring_requested_visibility(test_app) -> None:
    app, plane = test_app
    override_authenticated_identity(app, workspace_id="ws_1001", role_id="member", principal_id="member-1")
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        created = await client.post(
...
async def test_member_without_grant_gets_404_for_founder_document(test_app) -> None:
    ...
    docs = await plane.vault_repository.list_authorized_documents("ws_1001", "founder-1", {"founder"})
...
async def test_founder_can_review_reject_and_publish_flow(test_app) -> None:
    app, plane = test_app
    override_authenticated_identity(app, workspace_id="ws_1001", role_id="founder", principal_id="founder-1")
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        created = await client.post(
            "/agent/vault/documents", json={"title": "Plan", "media_type": "text/plain"}
        )
        ...
        claimed = await deps.local_repository.claim("ws_1001", body["upload_id"], "worker-task-token")
