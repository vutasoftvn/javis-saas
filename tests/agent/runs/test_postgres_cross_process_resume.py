...
pytestmark = pytest.mark.skipif(
    ...
    reason="AGENT_TEST_DATABASE_URL not set — skipping real-Postgres cross-process resume test",
)
...
from agent.runs.models import RunCheckpointRecord
...
async def test_cross_process_durable_resume_with_real_postgres(tmp_path: Path):
    ...
    from agent.runs.models import RunCheckpointRecord, RunRecord, RunApprovalRecord, RunToolCallRecord
    from agent.runs.repository import PostgresRunRepository
    from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine
