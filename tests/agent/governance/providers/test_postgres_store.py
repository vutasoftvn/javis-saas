...
async def test_save_manifest_entry_inserts_into_the_manifest_table():
    ...
    assert "INSERT INTO agent_governance.spec_resolution_manifest_entries" in sql
...
async def test_load_manifest_maps_rows_back_into_pinned_spec_identities():
    ...
    sql, params = session.executed[0]
    assert "FROM agent_governance.spec_resolution_manifest_entries" in sql
    assert params["run_id"] == "run-1"
...
from agent.governance.contracts import PolicyDecision, PolicyOutcome, RoleApproval
...
async def test_save_governance_state_upserts_state_and_appends_history():
    ...
    assert "INSERT INTO agent_governance.invocation_governance_state" in state_sql
    ...
    assert "INSERT INTO agent_governance.invocation_governance_history" in history_sql
...
from agent.governance.contracts import ApprovalEvidence
...
async def test_save_evidence_inserts_into_the_approval_evidence_table():
    ...
    assert session.committed is True
    sql, params = session.executed[0]
    assert "INSERT INTO agent_governance.approval_evidence" in sql
...
async def test_list_evidence_maps_rows_back_into_approval_evidence():
    ...
    sql, params = session.executed[0]
    assert "FROM agent_governance.approval_evidence" in sql
    assert params["scope"] == "call-1"
